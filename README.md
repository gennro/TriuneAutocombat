...
6. **Open Triune**: Triune automatically starts on login. If the window is closed or you need to re-open it, type `/ac` or `/lua run triune` in the chat bar.
   - Triune raises MQ2Lua's instruction budget (`/lua conf turboNum 10000`, saved in `config/MQ2Lua.yaml`) the first time it starts. MQ2Lua's default of 500 suspends every Lua script after 500 instructions and resumes it from the game loop, which makes spawn scans, chat and the map 10-50x slower than they should be. A value you set higher yourself is left alone.
...
- **Mesh Isolation Recovery**: A character standing on an unlinked navmesh island (a rock top, an unstitched stair polygon, a spot whose nearest polygon is under the floor) still reads as "on mesh" to MQ2Nav, yet every `PathExists` to an NPC fails, so Hunter/Puller would reject every target and stand still forever. When every nearby NPC reports no path, and ground the character stood on moments ago (the *last-good trail*, sampled while `/nav` was actively moving it) or the camp anchor is unreachable too, Triune leaves the mesh on purpose: `/moveto` straight back to the last good ground, then `/stick` at the nearest NPC, then the back/strafe/jump maneuvers, re-checking the mesh after each leg; on recovery the unreachable list is cleared and the hole is logged as an active hazard hotspot so detours avoid it. When the known ground *is* reachable (the mob stands on unmeshed ground instead), a close mob in plain sight is approached directly. Settings -> Navigation **Mesh Isolation Recovery** (on by default); the Status tab's Anti-Stuck column shows the signal and the last result.
...
- **Plugins Included**:
  ...
  - `chat.lua`: **Chat Windows** - a chat window replacement: every chat line is captured through one catch-all event, classified into 46 channels by pattern, and shown in any number of popout windows with filtered tabs, colours, timestamps, highlights, mutes, per-tab logs and an input line with inline clickable item links; `/tacchat` and `/ac chat` toggle it.
  ...
  - `nmsloot.lua`: **NMS Loot** - the server's personal loot system (`#nms`) as a window with a memory and a network: it types the `#nms` commands (`/say #nms ...`) and reads the replies from chat, so it knows who holds the **active looter** slot, whether loot echo is on and which items are offered to this character; over the Box Network every box shares that, so the window shows which of your boxes is the looter, a **Claim** button per box moves the slot, and any box's offered items can be listed and acted on (Keep / Sell / Tribute / Bank / Vault / Destroy / Pass to a player / Take coin) from one place. A status that reports offers waiting fetches the list by itself. A **compact** window (`/ac nms compact`, the Compact button, or the right-click menu) fits in a corner: who holds the slot, one chip per box (the holder lit, click a chip to claim there), the first few offers with one-letter K / S / T / B / V / D / P buttons, and Claim / Status / List / Coin / Echo buttons. Nothing it does is beyond the loot window; `/ac nms` toggles it.
...
- **Channels, not colours**: every incoming line (game text and Triune / MQ output alike) is sorted by its wording into one of 46 channels - Social (say, NPC dialogue, tells in / out, group, guild, raid, OOC, auction, shout, emotes, `/join` channels, pet chat, the `#server` commands Triune sends through `/say`), Combat (this server's abbreviated melee numbers and `miss`, your / others' / taken melee, crits, flurry-rampage, your pet, your / others' / taken spell damage, spell effect text, DoTs, damage shields, heals, casting, deaths, resists, combat messages) and Info (experience, loot & money, NMS loot - the `[NMS]` / `[NMS Loot]` lines: auto-sells, `#nms` replies, offers - item effects / upgrades, buffs landed / worn, skill ups, faction, consider, zone, system, Triune, MQ, unclassified). Anything the classifier does not know lands in **Unclassified** rather than disappearing; **Capture lines to file** (`/tacchat capture on`) writes every line with its channel to `logs/tac_chat_capture_<Name>.txt` so new patterns can be added.
...
- **Linking your own items**: link an item the way the game does - open it (right-click it in your inventory) and click its icon in the item window. The client writes the item's name into its own chat line (the link itself is kept as a tag MQ cannot read); the plugin matches that name to the open item window for its id, builds the link (the same one the Game Database's **Link to chat** sends), places it in the Chat Windows input as `[Name]` ready to send as a real clickable link, and takes the name back out of the game line (whatever you typed there stays). **Item window icon links go to this input** in General settings (or `/tacchat gamelinks on|off`) turns it off; `/tacchat gamelinks status` reports what the watcher sees (the window MQ found, the game line's text, the displayed items and their ids), and `/tacchat gamelinks window <name>` points it at another window name.
- **Tells that did not arrive**: a tell to someone offline (or to a mistyped name) gets no `You told ...` echo, only the game's `Bob is not online at this time.` The plugin files that as an undelivered outgoing tell to Bob, so it shows in Bob's conversation tab, the Tells tab and the tab you sent from, in the warning colour, and the status line above the input says the tell was not delivered.
- **Input history**: Up and Down in the input line walk the last 100 lines you sent, shell style - newest first, a line sent again moves to the front, what you were typing is set aside on Up and comes back on Down past the newest line, and a recalled line that carried item links sends them again. Kept per character with the config, so it survives a reload.
- **@Name completion**: type `@` and the first letters of a player's name and the matching names open in a list above the input, newest first - everyone whose chat you have seen (senders of say, tells, group, guild, raid, OOC, auction, shout; not NPCs, pets or you), people you exchanged tells with, and your group. **Tab** completes the selected one, **Up / Down** choose (they walk the sent-line history when no list is open), a click on a row works too, and **Enter** takes the completion instead of sending. The `@` is dropped, so `/t @bo` becomes `/t Bob `. **@Name completes player names** in General settings or `/tacchat complete on|off` turns it off.
- **The game's chat line gives the keyboard back**: with *Enter opens the chat input* on, the Enter that opens the Triune input also opens the game's own chat line, and the game never sees the Enter that sends from here - so its line used to keep the keyboard and eat your movement keys until you pressed Enter or Esc with the game. The plugin now tracks the Enter and Esc keystrokes the game sees (and text turning up in the game line), and whenever the game line opens that way it is emptied and closed with a keypress, so movement works again as soon as the Triune input lets go. **Close the game's chat line when Enter opens this input** in General settings (or `/tacchat gameline on|off`) turns it off if you type in the game's line yourself.
...
- Commands: `/tacchat show|hide|toggle|focus|settings|clear|tab <name>|tabs|window new|close <name>|mute|unmute <name>|timestamps|capture on|off|history on|off|clear|gamelinks on|off|status|window <name>|gameline on|off|complete on|off|ghost [window]|stats|reset` (also as `/ac chat ...`).
...
- **NMS Loot Across Boxes**: the NMS Loot plugin (`/ac nms`) shares who holds the server's **active looter** slot between your boxes, moves it with a **Claim** button per box, and lists / acts on any box's offered items from one window (see *NMS Loot* under Bonus Tools).
...
| 💬 **Chat Windows** | `/tacchat` or `/ac chat` | `chat.lua` plugin: chat window replacement - 46 pattern-classified channels, any number of windows with filtered tabs, colours, timestamps, highlights, mutes, per-tab logs, inline clickable item links and an input line. |
...
| 🎁 **NMS Loot** | `/ac nms` | `nmsloot.lua` plugin: the personal loot system's `#nms` commands as a window - who holds the active looter slot across your boxes, Claim per box over the Box Network, list any box's offered items and Keep / Sell / Tribute / Bank / Vault / Destroy / Pass them, loot echo on / off. |
...
| `/ac nms` | `/ac nmsloot` | Toggle the NMS Loot window (active looter slot across boxes, offered items) |
| `/ac nms compact` / `/ac nms full` | `/ac nms mini` | Switch to the compact NMS Loot window (looter, one chip per box, top offers) / back to the full one |
| `/ac nms claim [Box]` | | Take the active looter slot here (`#nms claim`), or on another box over the Box Network |
| `/ac nms status` / `/ac nms who` | | `#nms status` here and print what every box knows / ask the boxes and print the roster |
| `/ac nms list [Box] [handle]` | | `#nms list` here or on a box; the items show in the window with action buttons |
| `/ac nms loot <keep\|sell\|tribute\|bank\|vault\|destroy> "Item Name" [handle]` | `/ac nms keep "Item"` | Act on one offered item by name (`#nms loot ...`) |
| `/ac nms loot pass <Player> "Item Name" [handle]` | | Pass an offered item to a player |
| `/ac nms loot coin [handle]` | `/ac nms coin` | Take the coin (`#nms loot coin`) |
| `/ac nms echo on\|off` | | Print every loot offer to chat, one line per item (`#nms echo`) |
| `/ac nms <Box> <claim\|status\|list\|echo ...\|loot ...>` | | Run any of the above on another box |
...
│   │       └── nmsloot.lua      # NMS Loot plugin: active looter slot across boxes & #nms loot offers (/ac nms)
│   ├── config/
