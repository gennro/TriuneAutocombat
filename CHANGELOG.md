...
- **Box Network: Inter-Box Communication over MacroQuest Actors (`TAC/lua/tac/boxnet.lua`, `TAC/lua/triune.lua`, `README.md`, `tests/test_pure_logic.lua`).**
  - New `boxnet` plugin (Phase 1 of the multibox communication project) built on MacroQuest's native `actors` module: each eqgame client's post office talks over a named pipe to the `MacroQuest.exe` launcher, which routes messages between the boxes on that computer - no MQ2EQBC / EQBCS server, no MQ2DanNet, and typed Lua tables instead of string parsing. The plugin registers the `tac_boxnet` mailbox in `onInit` (re-tried every 5s if the module is missing or registration fails), unregisters it in `onDestroy` so `reloadPlugin` never double-registers, and its actor handler only appends to an inbox that `onTick` drains (the actors API forbids `mq.delay` inside a handler). Payloads pass through a sanitizer so an MQ datatype object can never reach the serializer, own broadcasts echoed back by the launcher are dropped by PID / character name, and messages carrying a different protocol version are ignored with a one-time warning per peer.
  - **Peer roster**: a heartbeat (name, level, zone, trio classes, mode / submode, running, burn, MA, HP / mana / end %, combat / sitting / casting, target id + name + HP, pet, position, pull state, Triune version) goes out every `heartbeatSec` (1s) and immediately when zone / mode / running / burn / MA / target / combat state change (min 0.25s spacing) or the plugin zones; a `hello` on start makes existing boxes answer at once and `bye` on destroy drops the box from every roster immediately. Peers silent for `peerTimeoutSec` (5s) expire.
  - **Remote commands**: `/ac net <all|zone|group|Name> <command>` (also `/ac boxnet`). The receiver re-validates the line and runs `/ac <line>` locally, so every existing command works across boxes; `zone` is a broadcast the receiver filters by its own zone, `group` sends to the peers in the sender's group, and a named send is an RPC whose reply (ran / refused + reason) or routing status (`RoutingFailed`, `NoConnection`, ...) is logged. Nested `net` / `boxnet` lines are refused on both ends. `/ac net peers`, `/ac net ping <Name>` (RPC round-trip, ms shown on the roster), `/ac net camp [scope]` (pushes the sender's location as `ctrl.camp_loc` + radius to trusted boxes in the same zone).
  - **Box Net window** (`ctrl.show_boxnet`, header button, Window Layout entry, `/ac net`): connection status with a launcher hint when sends fail or no peers ever appear, scope selector (All boxes / Same zone / My group), Run / Pause / Burn On / Burn Off / Follow Me (`ma <me>` + `assist chase`) / Set Me as MA / Camp Here, a free-form command box, the peer table (trio, zone, mode, state, HP / mana / end, target, last seen, per-row Run-Pause / Burn / Ping), and an event log.
  - **Trust** (plugin settings, persisted through `onSaveSettings`): *Accept remote /ac commands* (default on), *Only accept from the allowlist* (default off - any box on this launcher is trusted) with a comma-separated name list, *Print received commands to chat*, heartbeat interval and peer timeout sliders, default scope.
  - **`core.boxnet` API** for other plugins: `available()`, `myName()`, `peers()`, `peer(name)`, `command(scope, lines)`, `campHere(scope)`, `ping(name)`, `broadcast(kind, data)`, `send(name, kind, data, cb)` (RPC with `cb`), `subscribe(kind, fn(data, sender, message))` -> unsubscribe (`message:reply` answers an RPC). Groundwork for Phase 2 (MA target / engage broadcast consumed by Assist mode, peer counters replacing NetBots polling) and Phase 3 (buff / cure requests, group HUD from box vitals).
  - Core: `boxnet.lua` added to the discovery probe list, `/ac net` in the help table and usage line. README: feature section, plugin list, bonus-tools row, slash commands, file structure.
  - Tests: new Suite 95 runs three real plugin instances against an in-memory post office that mimics the launcher (mailbox matching, addressed vs broadcast delivery, echo-to-self, RPC replies, `RoutingFailed` / `AmbiguousRecipient` / `NoConnection`, serialization that counts non-plain values): discovery and rosters, heartbeat throttling and change triggers, broadcast / addressed / zone / group commands, receiver-side re-validation, allowlist and accept switch with RPC refusal reasons, Camp Here, the subscribe / broadcast / RPC API, expiry and `bye`, protocol mismatch, launcher offline, missing actors module, settings clamping, the `/ac net` surface, and render passes. Suites 87 / 88 / 89 include the sixteenth shipped plugin and its header button (3,381 tests passing).

- **Separate `has Poison` and `has Disease` Triggers (`TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - The single `has Poison/Disease` cure trigger fired on either affliction, so a poison-only cure (e.g. *Cure Poison*, *Purge Poison*) would be cast at a diseased target and vice versa. The Trigger dropdown now offers `has Poison` and `has Disease` individually; each checks only its own counters (`CountersPoison` / `CountersDisease`, `Me.Poisoned` / `Me.Diseased`, the Debuffs plugin, NetBots, the current target, and group members). The combined `has Poison/Disease` trigger remains available and existing profiles that use it keep working unchanged. Pet-target resolution for cures (`F: Pet`) picks the pet with the matching affliction.
  - Internally the monolithic `isPoisonedOrDiseased` walker is replaced by `hasAffliction(targetId, kind)` with `isPoisoned` / `isDiseased` wrappers; the legacy helper is now a thin `or` of the two.
  - New Suite 93 runs `hasAffliction` against a mock Me / group member with poison-only and disease-only counters, checks `conditionMet` routes each trigger (single target and `Whole Group`) to its own checker, and asserts both triggers are listed in the dropdown and help table (3,239 tests passing).

- **Auto AA: Post-Purchase Fireworks Summon Is Delayed (`TAC/lua/tac/auto_aa.lua`, `tests/test_pure_logic.lua`).**
  - After clicking Train on the fireworks AA the plugin issued `/alt act 17788` immediately and again 0.4s later, before the purchase had round-tripped the server, so the activation was a no-op. The Train step now calls `AA.scheduleFireworksSummon`, and `AA.processPendingFireworksSummon` (run from the plugin tick) fires the activation once the configurable delay has passed (`ctrl.auto_summon_delay_sec`, new **Summon delay (s)** slider next to Auto-Summon, default 3s, 0.5-15s), the character is idle, and `Me.AltAbility` confirms the rank landed - retrying on the same cadence up to 5 times before giving up. The periodic auto-summon stands down while a post-purchase summon is pending and uses the same delay as its minimum spacing.
  - Suite 90 covers scheduling, the wait, unconfirmed-purchase retries, firing once the rank is visible, the retry cap, and the 3s default (3,216 tests passing).

- **Multi-Pet Tracking: One Pet Per Class, No Duplicates, No Re-Summon Loops (`TAC/lua/triune.lua`, `TAC/lua/tac/hud_unitframes.lua`, `README.md`, `tests/test_pure_logic.lua`).**
  - `reconcilePets` and the main-loop `Me.Pet` watcher could hand a pet that one class already tracked to a second class (the reconcile only skipped IDs assigned *during that call*; the watcher fired whenever `Me.Pet` flipped to a surviving pet after another died). The Pets tab, Controls vitals, and the Target & Player HUD then showed the same pet twice, and the duplicated class's `missing pet` gem never fired. `petState.myPets` now keeps two invariants through new `trackPet` / `prunePetTracking` / `petTrackedCls` helpers: an ID is tracked under at most one class, and every tracked ID is a living pet of ours. `getMultiPetList` additionally deduplicates slot and extra pets by name (unique per player on this server), and the redundant `Me.Pet` append in the vitals lists is gone.
  - `missing pet` is now `isPetMissingForClass(cls)`: it reconciles (throttled, silent), then asks only whether *this gem's class* has a living tracked pet. A `missing pet` / `pet` gem cast calls `beginPetSummon`, which snapshots nearby pets and holds the class quiet for 12s; `updatePetTracking` (main loop) pins the first new pet of ours to the casting class - `Me.Pet` no longer has to change, which it usually does not for the second and third pet here - and learns its name into `ctrl.pet_names` (persisted with the loadout, used by `reconcilePets` and `isSpawnMyPet` to map pets back to classes after a restart or zone). A pet-typed spawn whose master the client cannot resolve is accepted only when it appears beside us after the cast; plain NPCs are never taken as pets. `You cannot have more than one pet` / `already have a pet` moves the least-certain tracked pet to the refused class and pauses that class's summons for 60s. `getAllMyPets` caches for 0.25s.
  - New Suite 92 runs the real functions against a mock spawn world: ownership rules, duplicate prevention, name dedupe, learned-name reconcile, per-class missing checks, summon grace and expiry, detection without a `Me.Pet` change, the `Me.Pet` flip after a pet death, refused summons, and single-pet-class trios (3,201 tests passing).

- **Auto-Accept Is a Popout Window (`TAC/lua/tac/auto_accept.lua`, `TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - The Settings -> Auto-Accept sub-tab (and the core's `UI.drawAutoAcceptSettings` delegation) is gone. The full rules + whitelist page now renders in the `Triune Auto-Accept` popout window driven by `ctrl.show_auto_accept`: header button (after Inventory), `/ac autoaccept` / `/ac acceptwin`, a Window Layout entry, and a Window toggle plus the three quick action checkboxes on the plugin's Configure panel.
  - Tests: Suite 55 asserts the sub-tab is gone and the window exists; Suite 88 covers the `/ac autoaccept` toggle; Suite 89 counts fourteen window-owning plugins (3,136 tests passing).

- **Auto AA: MQ2AAspend INI No Longer Rewritten on Every Settings Click (`TAC/lua/tac/auto_aa.lua`, `tests/test_pure_logic.lua`).**
  - `onLoadoutSaved` re-synced `Server_Character.ini` and issued `/aaspend load` on every `saveLoadout` - which fires for every window toggle, checkbox, or header button - whenever MQ2AAspend delegation was on (its default), even with Auto-Spend off. MQ2AAspend therefore kept reloading while Auto AA was idle. The hook now only syncs while **Auto-Spend AA** is enabled *and* delegated to MQ2AAspend, and `syncAAsToMQ2AASpendIni` fingerprints the generated `[MQ2AASpend_Settings]` / `[MQ2AASpend_AAList]` sections and skips the file write and plugin reload when nothing changed (priorities, buy order, mode, threshold). The manual **Sync to INI** button and `/ac aaspend sync` pass `force` and always rewrite. The fingerprint resets on plugin init so a character swap re-syncs once.
  - New Suite 91 runs the real plugin against a temp config dir: no sync while Auto-Spend is off, one sync on the first save, no-op on repeated saves, a re-sync on a real change, forced manual sync, and no sync when delegation is off (3,127 tests passing).

- **Spell Gem Bar Opens the Spellbook Plugin (`TAC/lua/tac/hud_spellgems.lua`, `tests/test_pure_logic.lua`).**
  - The gem bar's five "Open Spellbook" entry points (toolbar menu, per-gem context menu, empty-gem click, the spellbook button, and its right-click menu) still ran `core.toggleTool('triune_spellbook')`, i.e. `/lua run` on the script that was removed when the Spellbook Browser became a plugin, so nothing happened. They now call `openSpellbook()`, which opens the `spellbook` plugin window through `pm.setWindowOpen` (falling back to `ctrl.show_spellbook` if the manager is unavailable).
  - Suite 89 asserts no plugin launches a removed standalone script and that the gem bar's `openSpellbook` opens the spellbook window (3,115 tests passing).

- **Settings -> Windows Discovers Plugin Windows Automatically (`TAC/lua/triune.lua`, `TAC/lua/tac/*.lua`, `tests/test_pure_logic.lua`).**
  - The Window Layout manager's static `runtime.MANAGED_WINDOWS` list (thirteen hand-written plugin entries) is replaced by `runtime.CORE_WINDOWS` (Main, Mini HUD) plus `runtime.getManagedWindows()`, which appends an entry for every enabled plugin that declares `plugin.window`. A plugin window therefore gets position save / restore / center, Show / Hide, and visibility restore on the Windows page just by declaring itself - nothing to register in the core. Entries are cached per plugin so the page does not rebuild closures every frame; a disabled or errored plugin's window drops out of the list and returns on re-enable.
  - `plugin.window` gained optional `key` (position key used with `core.preBeginWindow`, defaults to the plugin id), `lockFlag` / `getLock` + `setLock` (Locked column), `desc` (row tooltip), and `defaultPos` (used by *Reset to Defaults*). The HUD plugins declare their existing keys and lock flags (`unit_frames`/`uf_lock`, `group`/`gw_lock`, `effects`/`eff_lock`, `cooldowns`/`cooldown_locked`, `xtarget`/`xt_lock`, `spell_gems`/`gem_lock`) so saved positions and locks carry over unchanged.
  - Tests: Suite 81 asserts the core no longer hardcodes plugin layout entries; Suite 89 builds the registry from the real plugins and checks key mapping, lock and open toggles, descriptions, and disable / re-enable behaviour (3,096 tests passing).

- **Plugins No Longer Contribute Main-Window Tabs: Auto AA Is a Popout Window (`TAC/lua/tac/auto_aa.lua`, `TAC/lua/tac/hud_cooldowns.lua`, `TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - The `onDrawTab` hook and `pm.drawTabs()` are gone; plugins integrate through the window system instead (a `window` declaration -> header button, Window Layout entry, `/ac` toggle). **Auto AA** is now the `Triune Auto AA` popout window driven by `ctrl.show_auto_aa` (header button between Cursor and Cooldowns, `/ac aawin` / `/ac aaui`, a new *Auto AA Spender* Window Layout entry, and a Window toggle on its plugin settings panel); the tab content is unchanged. **Cooldowns** drops its duplicate main-window tab and keeps only the popout Cooldown Monitor.
  - Tests: Suite 73/88/89 expectations moved from tab dispatch to the window contract (`pm.drawTabs` absent, `onDrawTab` undefined, `/ac aawin` toggle, thirteen window-owning plugins) (3,073 tests passing).

- **Plugin Windows Get Their Header Buttons from the Plugins Page (`TAC/lua/triune.lua`, `TAC/lua/tac/*.lua`, `tests/test_pure_logic.lua`).**
  - New plugin contract field `plugin.window = { label, tooltip, flag | isOpen()/setOpen(), headerButton, order }`. The manager gained `pm.getWindow / isWindowOpen / setWindowOpen / toggleWindow / headerButtonEnabled / setHeaderButton / windowPlugins / drawHeaderButtons`, and the main window's toolbar now renders one highlighted toggle button per enabled plugin window (wrapping to a new row when the header runs out of width) instead of eleven hardcoded buttons. Which plugins get a button is chosen per plugin with the new **Header** column on Settings -> Plugins (also a *Header Button* checkbox in the Configure modal); the column also carries a Show/Hide shortcut for the window. The preference persists as `ctrl.plugins[id].headerButton` and defaults to the plugin's `window.headerButton`.
  - All twelve window-owning plugins declare their window (Spellbook, Map, DPS, Cursor, Cooldowns, Target & Player HUD, Group, Effects, XTarget, Gems, Inventory, Buffbot) with `order` values that reproduce the previous header layout; Buffbot defaults to off since it never had a header button. Plugins without a window (Auto AA, Auto-Accept, Floating Damage) show "—" in the column.
  - Suite 89 covers the window contract (flag and callback forms), default / saved header preferences, ordering, open / close through the manager, the header renderer (counts, click-to-toggle, disabled / errored plugins excluded), and `collectSettings` persistence; Suites 75-79 now assert the plugins' window declarations instead of the removed hardcoded buttons (3,062 tests passing).

- **Auto AA: Fireworks Auto-Summon Waits Until the AA Is Purchased (`TAC/lua/tac/auto_aa.lua`, `tests/test_pure_logic.lua`).**
  - With *Auto-Summon Fireworks* enabled the plugin fired `/alt act 17788` every 3 seconds regardless of whether the character owned the AA, so characters that had not bought *Alternately Advanced Fireworks* yet spammed a no-op activation. New `AA.hasFireworksAA(aaId)` checks ownership through `Me.AltAbility(...).Rank() > 0` / `Me.AltAbilityReady(...)` (by the configured cap-spender name, `Summon Firework`, and the AA id); `checkAutoSummonFireworks` returns early until it is true and the manual *Summon Fireworks* button / `/ac summonnow` print a notice instead of activating. The post-purchase summon inside the train workflow is unchanged (the AA has just been bought there).
  - New Suite 90 loads the real plugin against a mock `Me.AltAbility` and verifies no `/alt act` before purchase, one activation after, the 3s cadence, and `AltAbilityReady` as proof of ownership.

- **Remaining Companion Scripts Migrated to Plugins: Map, DPS Parser, Inventory, Buffbot, Cursor (`TAC/lua/tac/map.lua`, `dps.lua`, `inventory.lua`, `buffbot.lua`, `cursor.lua`; `TAC/lua/triune_map.lua`, `triune_dps.lua`, `triune_inv.lua`, `triune_buffbot.lua`, `triune_cursor.lua` removed; `TAC/lua/triune.lua`, `tests/`).**
  - All five `/lua run` satellites (~11.6k lines) are now in-process plugins driven by new `ctrl.show_map` / `show_dps` / `show_inv` / `show_cursor` / `show_buffbot` flags. The header toolbar, Mini HUD, Settings -> External Tools buttons, and a new Window Layout manager entry per tool flip those flags instead of launching scripts; `/ac map|track|zone`, `/ac dps`, `/ac inv|bank`, `/ac cursorui`, and `/ac buffbot` are handled by the plugins' `onCommand`. Each plugin kept its feature code verbatim and dropped only what it duplicated from the core (the theme block, `mq.imgui.init`, its own main loop) - the per-character config files (`triune_dps_<name>.lua`, `triune_buffbot_config.lua`, `triune_map_config.lua`, the bank cache) are read and written exactly as before, and the config key is resolved once per `onInit` so a character swap (`restartAll`) never files the old character's settings under the new name.
  - **Fiber-aware `core.delay(ms, cond)`** (`pm.delay`): the buffbot casting workflow and the inventory bag-move / combine / sort sequences are sequential and used to sleep with `mq.delay`. Inside a plugin fiber `core.delay` now yields back to the main loop every tick until the time elapses or the condition holds, so those workflows keep their step-by-step shape without ever blocking the combat loop (outside a fiber it degrades to `mq.delay`). `pm.tick` marks `pm.inFiber` around each fiber resume; plugins must not call `mq.delay` or `mq.doevents` themselves (the core pumps events).
  - **`map.lua`**: camp / hunter anchor / waypoint / zone-hazard overlays are mirrored from the live `core.ctrl` (the script re-parsed `triune_loadout.lua` from disk every 2.5s and searched a dozen candidate paths; ~120 lines dropped). Map files and the atlas registry load lazily the first time the window opens, the engine tick runs only while the window is open, `onZoned` forces an immediate zone reload, and the MQ2Nav / MQ2MoveUtils autoload no longer blocks on `mq.delay`. `/ac track|tracker|trackui` opens straight onto the NPC Tracker tab.
  - **`dps.lua`**: keeps `/dps` and `/triunedps` (bound in `onInit`, unbound in `onDestroy`) and all 20 combat-log events (registered / released the same way). The parser runs whenever the plugin is enabled, so fights are tracked while the windows are hidden; the fight-timeout / target-death archiving that lived in the script's main loop is the plugin tick.
  - **`buffbot.lua`**: the station is **off after load** and is started with the new *Buffbot Station* button on the Controls tab, the plugin settings checkbox, or `/ac buffbot on|off|toggle` (bare `/ac buffbot` toggles the window) - the standalone script was implicitly "on" whenever it ran, which is not appropriate for an always-loaded plugin. Stopping clears the queue and pending offers. While a buff job is being cast the plugin holds the combat loop via `wantsCombatHold`. Tell / hail events are released on destroy.
  - **`inventory.lua`**: first scan runs when the window is opened (not at core start-up); queued actions and the background auto-scan drain through the plugin fiber.
  - **`cursor.lua`**: rewritten as a per-tick state machine (one `/autoinventory` per tick instead of a 255-iteration sleep loop); the continuous auto-clear preference persists through `onSaveSettings` / `onLoadSettings`.
  - **Core**: `UI.toggleTool` stays (exported as `core.toggleTool` for third-party scripts) but nothing in the core launches a satellite any more; the `/ac` usage line and help table list the plugin commands; `pm.discover`'s known-plugin probe includes the five new files. `triune.lua` grows by ~90 lines of wiring (window flags, layout entries, `pm.delay`).
  - **Tests**: the DPS / buffbot / map / inventory cross-module suites target the plugin files; `tests/check_theme_consistency.sh` now verifies that the core is the only theme (no plugin defines `pushTheme` or re-pushes the canonical colour tuples, and every non-overlay window uses `core.pushTheme`); Suites 87/88 include the five plugins in the shipped-plugin contract and integration run; new Suite 89 covers the cursor state machine, DPS event/bind lifecycle and fight timeout, inventory lazy scan + queued actions, buffbot station on/off + combat hold, map overlay sync from the live ctrl (character vs zone waypoints, hazards), the `/ac` toggles, and `pm.delay` fiber semantics (3,016 tests passing).

- **Spellbook Browser Migrated from a Standalone Script to a Plugin (`TAC/lua/tac/spellbook.lua`, `TAC/lua/triune_spellbook.lua` removed, `TAC/lua/triune.lua`, `tests/`).**
  - `triune_spellbook.lua` (1,403 lines, launched with `/lua run`) is now the in-process `tac/spellbook.lua` plugin (~900 lines). The browser UI, category / SPA classification, per-class level resolution, spell inspection, and the mem-to-gem queue are carried over verbatim; everything the script had duplicated from the core was dropped in favour of the plugin API: theme (`core.pushTheme/popTheme`), the spell database (`core.DATA` instead of its own `loadData`), class detection (`core.myClasses`, with the Re-detect button now requesting the core's re-scan via `core.requestClassRedetect`), the spellbook map / slot lookup (`runtime.getSpellBookSlot`), and memorization (`runtime.tryMem`, so both the combat loop and the browser memorize through the same code path). The queue drains one gem per tick on the main coroutine with the same combat / moving / casting gates the core uses.
  - Window visibility is `ctrl.show_spellbook`: the header **Open Spellbook** button, the Settings -> External Tools button, `/ac spellbook` / `/ac book` (handled by the plugin's `onCommand`), and a new **Spellbook Browser** entry in the Window Layout manager all flip it. The plugin's version stamps the window title like the other HUDs.
  - Core API exports added: `cleanSpellName`, `normalizeSpellName`, `requestClassRedetect`.
  - `tests/check_theme_consistency.sh` no longer lists the removed satellite; the 200-locals check covers the new plugin files; Suite 88 covers the `/ac spellbook` toggle, class-key casing in `getClassSpells`, and the queue's combat gate + `runtime.tryMem` pass-through (2,785 tests passing).

- **Extended Target HUD Hides Pets and Friendly Players (`TAC/lua/tac/hud_xtarget.lua`, `tests/test_pure_logic.lua`).**
  - The XTarget popout listed every populated XTarget slot, so the character's own pets (including the Gestalt Trio's extra pets), group pets, mercenaries, and PC role slots (Specific PC, Group Tank/Assist/Puller, Raid Assist) showed up alongside hostiles. Slots are now filtered by spawn type, XTarget slot type, and the multi-pet list from the core; `... Target` role slots (e.g. `Group Tank Target`, `My Pet Target`) are kept because they point at what the friendly is fighting.
  - New right-click options **Show Pets & Mercenaries** (`ctrl.xt_show_pets`) and **Show Friendly Players** (`ctrl.xt_show_pcs`), both off by default, restore the old behaviour when wanted.
  - Suite 78 covers the filter matrix (own pet, group pet, Trio pet by id, mercenary, PC role slots, `... Target` slots, and both opt-in toggles).

- **Cooldown Monitor and Spell Gem Bar Migrated to Plugins (`TAC/lua/tac/hud_cooldowns.lua`, `TAC/lua/tac/hud_spellgems.lua`, `TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - **`tac/hud_cooldowns.lua`** carries `getTrackedCooldownItems` (557 lines), `renderCooldownContent` (423) and the popout window, and contributes the **Cooldowns** main-window tab through `onDrawTab` (so the tab keeps its place right after Auto AA - both are plugin tabs now, drawn in plugin load order). It is read-only against combat state (`runtime.lastCast`, `discExpires`, `discCooldown`, `timerGroupCooldown`, `last*FiredAt`, cast tracker) and fires abilities through the same `runtime.fire* / castGem / useClickie` entry points the combat loop uses. The search box state (`cooldownSearch`) moved off `runtime`.
  - **`tac/hud_spellgems.lua`** carries `drawSpellGemBarWindow` (531 lines). The per-gem countdown state (`gemCooldownEnd` / `gemCooldownSpell`) and the spell-set name input (`newSpellSetName`) were only ever used by the bar, so they are plugin-local now instead of living on `runtime` / `UI`.
  - **Core API exports** added for these: `parseSpellRecastTime`, `parseCombatAbilityTimer`, `getAbilityBaseCooldown`, `getDiscCooldownAndDuration`, `getNumGems`, `classColor`, `col32`, `toVec`, `toggleTool`, `getSpellIconAnimation`, `getGemCooldownSec`, `drawSpellbookIcon`, and a live `castTracker` getter.
  - Removed `UI.drawCooldownsTab`, `UI.drawCooldownWindow`, `UI.drawSpellGemBarWindow` and their `mq.imgui.init` registrations; `triune.lua` is now 19,991 lines (from 21,551).
  - **Tests**: Suites 79 and 81 and the Cooldowns-tab ordering checks target the plugin files; Suites 87/88 include the two new plugins in the shipped-plugin contract and integration run (2,729 tests passing).

- **Auto AA Migrated to a Plugin (`TAC/lua/tac/auto_aa.lua`, `TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - **`tac/auto_aa.lua` (2.8k lines)** now owns the entire AA engine that used to live in the core: AA window helpers (`getAAWindow` / `openAAWindow` / `closeAAWindow` / `findChildRecursive`), the class-restriction catalog (`CLASS_ARCHETYPES`, `ARCHETYPE_RESTRICTIONS`, `CLASS_SPECIFIC_ABILITIES`), `scanPlayerAAs` / `recordScannedAA` / `getFilteredSortedAAs`, the native window trainer (`startAATrainWorkflow` / `processAATrainWorkflow` / `findAAInWindowLists`), `checkAutoSpendAA` / `manualSpendAA`, MQ2AAspend detection and INI sync, the Fireworks cap spender and auto-summon, the `You have purchased/improved/mastered` chat events, the **Auto AA** main-window tab, and the `/ac autoaa | autofw | spendnow | summonnow | aathreshold | aacost | aaid | aaname | aascan | aaprio | aaspend` commands. All AA scan state moved off `runtime.*` into the plugin (`runtime.cachedAAData` stays shared because the core AA loadout tab tooltips read it). `triune.lua` drops to 21.5k lines (from 24.1k).
  - **New plugin hooks** so the plugin can integrate without reaching into the combat loop: `onDrawTab()` (contribute a tab to the main window; dispatched via `pm.drawTabs()` where the Auto AA tab used to be), `wantsCombatHold()` (the puller / assist loop stops moving while it returns true - replaces the five `runtime.pendingAATrain` checks with `runtime.combatHold()`), `onBetweenPulls()` (the puller's between-pull gap; replaces the two inline `checkAutoSpendAA(true)` calls), `onCommand(cmd, args)` + `plugin.help` (the `/ac` dispatcher falls through to plugins before the usage message, and `/ac help` prints plugin help lines), and `onLoadoutSaved()` (the MQ2AAspend INI sync now runs from this hook instead of inside `runtime.saveLoadout`). Hook errors flag the plugin as `Error` like every other hook.
  - **Core API**: `myClasses` is now a live getter on the core API alongside `ctrl` / `loadout`.
  - The MQ2AAspend startup autoload was removed from the core's `autoloadRequiredPlugins`; the plugin still autoloads it on demand when Auto-Spend is active.
  - **Tests**: the ~100 source assertions in Suites 62-73, 82, 84 and 85 now target the plugin file (`AA_CONTENT`), Suite 73 verifies the new `combatHold` / `onBetweenPulls` wiring on both sides, Suite 87's shipped-plugin contract includes `auto_aa`, and Suite 88 exercises the real dispatcher for combat hold, between-pulls, `/ac` commands, help lines, tab drawing, loadout-saved, and hook-error recovery (2,700 tests passing).

- **Plugin Engine Hardening & Second Wave of Plugin Migrations (`TAC/lua/triune.lua`, `TAC/lua/tac/`, `tests/test_pure_logic.lua`).**
  - **Auto-Accept is now owned entirely by `tac/auto_accept.lua`**: the core's duplicate engine (`runtime.checkAutoAccept`, `runtime.isAutoAcceptAllowed`, the whitelist helpers, five `TriuneAuto*Invite` chat events, and the 358-line Settings -> Auto-Accept page) was removed. Previously both the core and the plugin were clicking the same dialogs, and the plugin copy had dropped the "Always accept from Group Members" rule and the `Me.Invited` / `TradeWnd.HisTradeReady` polling. The plugin now carries the full original logic (events + 0.3s poll + debounce), and Settings -> Auto-Accept renders the plugin's own panel via the new `pm.drawPluginSettings(id)`.
  - **Group, Effects & Songs, and Extended Target popouts migrated to plugins** (`tac/hud_group.lua`, `tac/hud_effects.lua`, `tac/hud_xtarget.lua`). All three are render-only (`hasThread = false`) and keep using `ctrl.show_*_window`, so the header buttons, Mini HUD toolbar, Window Layout manager, and `/ac group|eff|xtar` commands are unchanged. `UI.drawGroupWindow`, `UI.drawEffectsWindow`, `UI.drawXTargetWindow` and their `mq.imgui.init` registrations were removed (`triune.lua` is now 24.1k lines, down from 25.9k).
  - **Live `ctrl` for plugins (`pm.getCoreApi`)**: the core API is now a single shared table whose `ctrl` / `loadout` / `petState` / `pursuit` / `stuckState` fields resolve through `__index` getters. `runtime.onCharacterChanged()` replaces `ctrl` wholesale, so the previous snapshot left every plugin reading and writing a dead config table after a character swap. The main loop now also calls `pm.restartAll()` after a swap so plugins re-seed defaults and re-read their enabled / combat flags from the new character's config. Also exported: `drawSpellIcon`, `addIgnore`, `parseDurationSec`, `fmtSec`, `idxOf`.
  - **Rescan no longer double-initialises**: `pm.discover()` skips files that are already registered, so "Rescan Plugins Folder" cannot run `onInit` on a live instance without `onDestroy`.
  - **Reload keeps enabled plugins enabled**: `pm.reloadPlugin` restored the user's enabled flag before re-loading; previously `disablePlugin` persisted `enabled=false` and the reload read it back, leaving the plugin disabled.
  - **Render errors are contained**: an exception inside `onDrawUI` now flags the plugin as `Error` (visible on the Plugins tab) instead of being silently swallowed and retried every frame with an unbalanced ImGui stack. `onDrawSettings` errors are handled the same way.
  - **Fiber creation factored into `pm.createFiber`** (was duplicated in `enablePlugin` and `tick`).
  - **`hud_unitframes`** no longer forces `show_unit_frames = true` on every load (the saved preference is respected), and `refreshVitals` is throttled to 50ms whether it is driven by the fiber or the render pass instead of re-polling every TLO on every frame.
  - **`floating_damage`** honors the existing "Critical Hit Floating Text" checkbox (`ctrl.show_crit_floaters`), which had become a no-op after the overlay moved out of the core; the toggle is also exposed in the plugin's own settings panel.
  - **`auto_accept`** no longer double-persists its options through `onSaveSettings` / `onLoadSettings` (the `ctrl.*` keys already round-trip with the loadout).
  - **Tests**: Suite 55 now exercises the real `auto_accept.lua` authorization / whitelist logic against a mock core instead of a hand-written mirror; Suites 76-78 and 81 target the plugin files; new Suite 87 pins the manager hardening and the shipped-plugin contract; new Suite 88 loads the real `runtime.initPluginManager` in a sandbox against the shipped plugins and covers discovery, combat sleep, disable/enable, character-swap `restartAll`, `collectSettings`, draw-error isolation, `drawPluginSettings`, and reload (2,667 tests passing).

- **Restore Buttery-Smooth Real-Time 60 FPS Live Updates to Unit Frames HUD (`TAC/lua/tac/hud_unitframes.lua`).**
  - **Direct Render-Pass Vitals Refresh (`refreshVitals`)**: Called live vitals refresh directly inside `plugin.onDrawUI` on the ImGui DirectX render thread. Prevents the HUD from lagging or freezing during spell casts and `combatTick` delays on the main thread, restoring identical real-time responsiveness to when the HUD was embedded in `triune.lua`.
  - **ImVec2 Type Resolution**: Fixed `ImVec2` type resolution in auto-attack pulsing border drawing to prevent silent pcall failures.

- **Interactive In-Combat Execution Toggle & Centered Modal Configuration Popup (`TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - **In-Combat Table Toggle Column**: Added a dedicated `Combat` column to the Plugins table with a 1-click checkbox. Users can immediately toggle whether any plugin's coroutine fiber updates during combat or pauses/sleeps during combat to eliminate combat latency. Setting persists to `ctrl.plugins[id].runInCombat`.
  - **Centered Modal Configuration Dialog (`PluginConfigModal`)**: Replaced the cumbersome bottom-of-page settings scroll with an on-demand centered modal dialog (`ImGui.BeginPopupModal`). Clicking `Configure` on any plugin (regardless of table size) immediately displays its settings, status, combat toggle, and custom controls directly in front of the user with an embedded scrollable child frame and a dedicated `Close` button.
  - **Modal Open Trigger & Scope Isolation**: Fixed modal activation by synchronizing `ImGui.OpenPopup` outside table column scope (`pm.openConfigRequested`) with the exact popup identifier `'Plugin Configuration##PluginConfigModal'`, preventing premature ID dismissal.
  - **Updated Suite 86**: Verified persistence of `runInCombat` during `pm.loadPlugin` and `pm.collectSettings`, along with modal dialog registrations (2,511 tests passing).

- **Fix Plugin Auto-Discovery for `hud_unitframes.lua` & Instant HUD Window Visibility (`TAC/lua/triune.lua`, `TAC/lua/tac/hud_unitframes.lua`).**
  - **Comprehensive Multi-Path & File Probing (`pm.getPluginDir`)**: Probed candidate directories across `mq.luaDir`, `scriptDir`, and relative paths (`lua/tac`, `TAC/lua/tac`) against all known plugins (`hud_unitframes.lua`, `auto_accept.lua`, `floating_damage.lua`) rather than just `auto_accept.lua`.
  - **Dynamic OS popen Directory Scanning (`pm.discover`)**: Added `io.popen` directory query (`dir /b` on Windows, `ls -1` on Linux) to scan for all dropped `.lua` files even when `lfs` (LuaFileSystem) is not installed in MacroQuest.
  - **Exhaustive Built-in Fallback Probe**: Added `hud_unitframes.lua` and all standard modular plugins to the direct `io.open` fallback list in `pm.discover`, ensuring `hud_unitframes.lua` is always discovered and loaded regardless of environment.
  - **Instant HUD Window Visibility on Enable**: Set `core.ctrl.show_unit_frames = true` on `plugin.onInit` when enabled, and added a direct `HUD Window: Visible / Hidden` toggle button in the plugin's configuration panel in Settings -> Plugins.

- **Fix "Cannot create event ... it is already defined" on Plugin Reload & Monolith Cleanup (`TAC/lua/tac/floating_damage.lua`, `TAC/lua/triune.lua`).**
  - **Pre-Registration Event Unbinding (`mq.unevent`)**: Added `pcall(mq.unevent, name)` prior to calling `mq.event(name, ...)` in `floating_damage.lua`. Prevents MacroQuest from throwing red `Cannot create event TacCritHit, it is already defined` errors when plugins are reloaded or re-initialized across sessions.
  - **Embedded Floaters Removal**: Removed 256 lines of legacy floating damage code (`runtime.CRIT`, `runtime.spawnCritFloater`, `UI.drawCritOverlay`, and duplicate `TriuneCritHit` handlers) and `TriuneCritOverlay` from `TAC/lua/triune.lua`, eliminating duplicate floater spawns and reducing `triune.lua` to 25,770 lines with 183 local variables.

- **Unit Frames Popout HUD Autonomous Plugin Migration & Monolith Code Removal (`TAC/lua/tac/hud_unitframes.lua`, `TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - **Autonomous Plugin Extraction (`TAC/lua/tac/hud_unitframes.lua`)**: Extracted the Popout Unit Frames HUD (Target + ToT with aggro warnings, target buffs/debuffs with countdown timers, player vitals HP/Mana/End/XP/AAXP, and multi-pet bars for the Gestalt Trio) into an autonomous plugin in `lua/tac/`.
  - **Dedicated 50ms Fiber Snapshotting**: Runs on its own coroutine fiber (`tickInterval = 0.05`) to snapshot TLO vitals at 50ms intervals. The ImGui render thread reads from cached snapshots with zero TLO overhead, eliminating combat latency while keeping the HUD updating smoothly in combat (`runOutOfCombatOnly = false`).
  - **Embedded Monolith Code Removal**: Removed 412 lines of legacy embedded rendering code (`UI.drawUnitFramesWindow`) and `TriuneUnitFramesWindow` from `TAC/lua/triune.lua`, reducing the main script size to 26,018 lines while keeping main chunk local variables at 183 / 200.
  - **Core API Exposure (`pm.getCoreApi`)**: Extended `pm.getCoreApi()` to expose `VERSION`, `preBeginWindow`, `postBeginWindow`, `drawStatusProgressBar`, `getConColorRgb`, `resolveTargetOfTarget`, `getMultiPetList`, `getPetSpawnInfo`, and `isSpawnAlive` to autonomous HUD plugins.
  - **Independent ImGui Registration (`TriunePluginsUI`)**: Registered a dedicated top-level ImGui callback (`mq.imgui.init('TriunePluginsUI', UI.drawPlugins)`) so plugin windows and overlays render cleanly even when the main configuration window is closed or minimized.
  - **Updated Suites 75, 81, and 86 in `tests/test_pure_logic.lua`**: Verified plugin loading, lifecycle hooks, snapshotting, and window hook compliance (2,507 tests passing).

- **Modular Plugin System & Coroutine Fiber Engine (`TAC/lua/triune.lua`, `TAC/lua/tac/`, `README.md`, `tests/test_pure_logic.lua`).**
  - **Dynamic Drop-and-Play Discovery (`lua/tac/`)**: Created the dedicated plugin directory (`TAC/lua/tac/`) where users can drop any `.lua` plugin file. Automatically discovered at startup or via the "Rescan Plugins Folder" UI button with multi-path fallback and zero-process iteration.
  - **Dedicated Settings -> Plugins UI Tab (`UI.drawPluginsTab`)**: Added a new sub-tab under Settings with an interactive table displaying loaded plugins, enable/disable toggle checkboxes, status badges (`Active`, `Sleeping (Combat)`, `Disabled`, `Error`), author/version details, multiline descriptions, one-click reload buttons, and embedded plugin configuration panels.
  - **Execution Latency & Performance Profiler**: Integrated a live performance profiler on the Plugins tab displaying execution time in milliseconds (`Last: %.2f ms | Avg: %.2f ms`) with color-coded alerts (`< 1.0 ms` Good, `1.0–3.0 ms` Warning, `> 3.0 ms` Alert), enabling users to pinpoint CPU usage and latency sources immediately.
  - **Coroutine Fiber Threading ("Green Threads")**: Plugins run in their own dedicated coroutine fibers with customizable tick intervals (`tickInterval = 1.0`, `0.05`, etc.), completely removing heavy non-combat logic from the high-frequency combat loop.
  - **Automatic Combat Throttling & Zero Combat Latency**: Non-combat plugins with `runOutOfCombatOnly = true` automatically enter `Sleeping (Combat)` mode during active combat (`Me.Combat() or Me.CombatState() == 'COMBAT'`), guaranteeing 100% of CPU time is dedicated to targeting, casting, and navigation.
  - **In-Combat Popout HUD Support**: Popout and combat-assisting plugins (`runOutOfCombatOnly = false`) continue updating smoothly during combat in their own fibers without stalling `combatTick()`.
  - **Crash Isolation & Error Sandboxing**: All plugin invocations (`onInit`, `onTick`, `onDrawUI`, `onCombatTick`, `onZoned`) are safely wrapped in `pcall` containment guards; faulty plugins transition to `Error` state with stack traces accessible via hover tooltip without crashing Triune or EverQuest.
  - **Pilot Plugins Extracted**:
    - `TAC/lua/tac/auto_accept.lua`: Standalone plugin for automatic group, trade, and raid/DZ invite acceptance with whitelist management and Settings UI.
    - `TAC/lua/tac/floating_damage.lua`: Standalone plugin for 3D animated floating damage numbers on critical hits, crippling blows, deadly strikes, and spell crits.
  - **Canonical Version Bump to 2.15**: Bumped version across `TAC/lua/triune.lua`, `README.md`, and test suites.
  - **Added Unit Test Suite 86**: Added comprehensive unit tests in `tests/test_pure_logic.lua` covering plugin loading, fiber dispatch, combat sleep transitions, error isolation, and source code registration (2,496 tests passing).

- **Auto AA Alternately Advanced Fireworks Spending & Summoning Reliability Fix (`TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - **Immediate Post-Purchase `/alt act 17788` Auto-Summon (`runtime.processAATrainWorkflow`)**: Immediately upon purchasing the *Alternately Advanced Fireworks* AA in the AA window (during both `click_train` and `finish` steps), Triune now automatically dispatches `/alt act 17788` to summon fireworks and clears the cursor via `/autoinventory`.
  - **Unblocked Background Fireworks Auto-Summon (`runtime.checkAutoSummonFireworks`)**: Eliminated fragile `AltAbilityReady` checks that falsely evaluated to false on custom server abilities. Replaced with direct `/alt act 17788` dispatch gated only by combat/casting safety and active positive cooldown timers (`AltAbilityTimer`).
  - **Native `/alt act` Command Dispatch (`checkAutoSummonFireworks`, `manualSummonFireworks`)**: Updated all fireworks summon command dispatches from `/alt activate` to `/alt act %d`, matching the user's hotkey command and EverQuest alias.
  - **Special Tab Repeatable Gating in Priority Queue (`runtime.checkAutoSpendAA`)**: In the prioritized AA evaluation loop, allowed Special tab abilities (`isSpecial`) to bypass `canTrainCheck` (`local canTrainMet = isSpecial or canTrainCheck`). For repeatable Special AAs like Fireworks that are already owned at rank 1, MacroQuest's `CanTrain()` returns `false`, which previously caused prioritized Fireworks to be discarded as unpurchasable.
  - **In-Game AA Window Filter Unchecking (`runtime.processAATrainWorkflow`)**: In `prepare_tab` and `select_item`, detected whether EverQuest's `AAW_TrainFilter` (`CanPurchaseFilter` checkbox) is active. When checked, the client hides completed rank 1 abilities including repeatable Special AAs; Triune now automatically unchecks `AAW_TrainFilter` on Tab 4 (Special) and before failing selection, ensuring Fireworks is visible in `AAW_SpecialList`.
  - **Special Cap Spender Threshold Evaluation (`runtime.checkAutoSpendAA`)**: Improved fallback cap-protection threshold evaluation so that special cap spenders (Fireworks) trigger reliably whenever points meet or exceed the cost and configured threshold.
  - **Listbox Direct Lookup Aliases (`runtime.findAAInWindowLists`)**: Added direct listbox lookup for `=Summon Firework` and `Summon Firework` alongside `=Alternately Advanced Fireworks`.
  - **Unit Test Suite 85**: Added comprehensive unit tests in `tests/test_pure_logic.lua` covering post-purchase summon dispatch, cooldown timer checks, `/alt act` command dispatch, `canTrainMet` Special tab bypass, filter unchecking, and direct list lookup aliases (2,461 tests passing).

- **Fix Spell Gem Popout Recast Countdown 1194h Underflow Bug (`TAC/lua/triune.lua`, `tests/test_pure_logic.lua`).**
  - **Reject Unsigned 32-Bit -1 Sentinels (`0xFFFFFFFF` / `4,294,967s`)**: Fixed an issue where EverQuest's internal inactive recast timer value (`(DWORD)-1` = `4,294,967,295 ms`) was interpreted as a valid `4,294,967` second duration by `Me.GemTimer(slot)` / `parseDurationSec`, causing spell gem buttons to render `1194h` whenever a gem was not cooling down or during Global Cooldown (GCD).
  - **Upper-Bound Cooldown Guard**: Added strict upper-bound sanitization in `parseDurationSec` and `UI.getGemCooldownSec` rejecting values `>= 2,147,483,647 ms` (`0x7FFFFFFF`), `>= 2,000,000 s`, and any gem cooldowns exceeding maximum plausible spell recast (`math.max(3.0, spellRecast + 3.0)`).
  - **Spell Gem Button Overlay Sanitization**: Clamped gem button overlay formatting to minutes/seconds (`cdSec < 3600`), preventing hours formatting (`%dh`) on spell gem buttons.
  - **Recast Resolution with `parseSpellRecastTime`**: Switched spell gem recast lookup to `parseSpellRecastTime(g)`, correctly resolving spell definition recast times across all MacroQuest datatype variations.
  - **Automated Verification**: Added comprehensive unit tests in `tests/test_pure_logic.lua` (section F and Suite 79) validating 0xFFFFFFFF sentinel rejection, recast clamping, and gem overlay formatting (2,443 tests passing).
...
- **Authentic In-Game Inventory Screen Stats Tab Replication (`triune.lua`, `README.md`, `tests/test_pure_logic.lua`).** Re-engineered the **Stats** tab in the popout Character Window (`TriuneCharacterWindow`) to replicate the exact look, layout, alignment, and color palette of EverQuest's native in-game inventory screen stat page:
  - **Authentic Two-Column Stat Layout**: Mirrored the in-game UI screen arrangement with twin columns containing all 5 authentic sections:
    - **Current Status**: HP (`cur / max`), Mana (`cur / max`), Endurance (`cur / max`), Combat Mitigation (AC), Combat Evasion, Combat HP Regen, Combat Mana Regen, Combat End Regen, Attack, and Haste (%).
    - **Basic Stats**: Strength, Stamina, Intelligence, Wisdom, Agility, Dexterity, and Charisma, each with base value (green), stat cap (`/<cap>` in white), and heroic bonus (`+<heroic>` in gold).
    - **Spell Resists**: Magic, Fire, Cold, Disease, and Poison resists with current resist (green), cap (`/525` in white), and heroic/overcap bonus (`+<overcap>` in gold).
    - **Advanced Item Stats**: Complete Mod2 gear stats including Heal Amount, Spell Damage, Worn ATK, Combat Effects, Spell Shield, Shielding, Damage Shield, DoT Shielding, Damage Shield Mitigation, Avoidance, Accuracy, Stun Resist, and Strike Through, each showing active modifier (green if > 0), separator, and era cap (e.g. `/750`, `/850`, `/100`, `/35`, `/25`, `/150`).
    - **Advanced Character Stats**: Spell Crit Rate (%), Spell Crit Modifier (%), DoT Crit Rate (%), DoT Crit Modifier (%), Heal Crit Rate (%), HoT Crit Rate (%), Melee Crit Rate (%), Archery Crit Rate (%), and Physical Crit Modifier (%).
  - **Faithful Color Palette**: Applied authentic EQ color tokens (`UI.C_EQ_HEADER` soft slate-blue, `UI.C_EQ_LABEL` crisp off-white, `UI.C_EQ_GREEN` bright neon green for active/modified stats, `UI.C_EQ_WHITE` pure white for caps and unmodded values, and `UI.C_EQ_GOLD` for heroic bonuses).
  - **Dynamic In-Game Window Data Querying with TLO Fallback (`UI.readInvChildText`)**: Intelligently reads live client labels directly from EverQuest's native `InventoryWindow` child elements (`IWS_CurrentHP`, `IWS_CurrentArmorClass`, `IWS_CurrentStrength`, `IWS_CurrentMagic`, `IWS_CurrentHealAmount`, `IWS_CurrentSpellCritRate`, `IWS_CurrentPhysicalCritRatio`, etc.) with seamless fallback to `mq.TLO.Me` attributes and era-accurate caps when the native inventory window is closed.
  - **In-Game Window Stat Scanning & Caching (`runtime.scanStatsFromInventoryWindow`)**: Exactly like Trio class detection reads the player's 3 classes by opening and inspecting the native `InventoryWindow`, `scanStatsFromInventoryWindow` activates the Stats subwindow (`IW_Subwindows tabselect 2`), reads all 49 client-computed XML label pieces (`IWS_*`), and caches them into `runtime.cachedInvStats`.
  - **Asynchronous Main Loop Sync & UI Trigger**: Added a `[Sync Stats]` button in the Stats tab header, an `/ac char sync` (or `/ac char scan` / `/ac char refresh`) slash command, and opportunistic stat scanning during character class detection, all queued safely to execute on the main coroutine loop without blocking or stalling ImGui frames.
  - **Removed Redundant TLO Class from Header**: Omitted the single base class reported by `Me.Class` (which always returns `BRD` on custom trio emulator setups), displaying character name, level, and race alongside the detected Trio classes.
  - **Dedicated XP & AA XP Progress Bars**: Split the combined progress bar into two distinct, full-featured progress bars (18px tall to eliminate font clipping and vertical text cutoffs): warm golden-amber for regular Level EXP and iconic amethyst-purple for Alternate Advancement (AA) EXP.
  - **Native AA Window Toggle Button**: Added a dedicated `[AA Window]` button (sized to match progress bar height) next to the AA EXP bar that seamlessly opens or closes EverQuest's native Alternate Advancement window.
  - **Native Skills Window Toggle Button**: Added a dedicated `[Skills]` button in the Stats tab header next to `[Sync Stats]` to instantly open or close EverQuest's native Skills window (`runtime.toggleSkillsWindow`, `/ac char skills`, `/skills`).
  - **Narrower Worn Equipment Panel & Larger Aligned Slots**: Resized the worn equipment paperdoll panel in the Inventory tab to 215px (down from 400px), dedicating the remaining 590px+ of width to the personal inventory bags panel. Enlarged worn equipment slot cells from 34px to 48px (+41% larger) for high-fidelity icon display, and replaced dynamic same-line caption flow with fixed-column coordinate positioning (`colStep`) so text labels are centered and aligned directly beneath each corresponding slot without drifting or column overlap.
  - **Personal Inventory Bag Right-Click Context Menu & Destroy Confirmation Modal**: Replaced direct right-click inspection in personal inventory bag slots with an interactive context menu offering `Use / Click`, `Inspect`, and `Destroy...`. Selecting `Destroy...` opens an ImGui modal confirmation dialog warning of permanent destruction with item name, location, and stack count; confirming queues a safe multi-step pickup, cursor verification, and `/destroy` execution on the yieldable main loop.
  - **Automatic Stats Sync on Worn Equipment Change (`runtime.checkWornItemsChanged`)**: Implemented automatic monitoring of all 23 worn equipment slots (0..22). Whenever any worn equipment is changed, swapped, equipped, or unequipped (whether via drag-and-drop, native EQ inventory, hotkeys, or bandolier), a stats synchronization is automatically triggered and processed safely on the main loop once out of combat/casting.
  - **Fixed ImGui Style Color Stack Underflow in Theme & Destroy Modal**: Fixed a style color stack mismatch where `UI.pushCol` was called for local button styling in the Destroy Item modal popup, polluting `runtime.colN` and causing `UI.popTheme()` to pop extra colors from ImGui's stack (`ImGui Critical Failure: Missing PopStyleColor()`). Rewrote `UI.pushTheme()` and `UI.popTheme()` to be stack-safe using dedicated invocation counts (`runtime.themeColStack` / `runtime.themeVarStack`), ensuring clean and bulletproof popping across all concurrent top-level windows.
  - **Banked & Spent AA Display**: Added live counters showing `Banked` (unspent), `Spent`, and `(Total)` Alternate Advancement points with hover tooltips for lifetime point tracking.
  - **Precision Pixel & Right-Alignment Calculation (`UI.drawEqTextRight`)**: Implemented text width calculation (`ImGui.CalcTextSize`) to right-align values against slash dividers and caps, matching EverQuest's native SIDL XML grid layout down to the pixel.
  - **Informative Hover Tooltips**: Added descriptive tooltips to every stat row explaining what the modifier governs and its cap mechanics.
  - **Canonical Version Bump**: Bumped canonical version to **2.11** across `triune.lua`, `README.md`, and test suites.

- **Fixed Premature Dead NPC Evaluation at 0% Health (`triune.lua`, `tests/test_pure_logic.lua`).** Resolved a regression where living NPCs with 0% health / 0 HP were falsely evaluated as dead, causing characters to clear target, drop auto-attack, ignore active enemies on XTarget, and wander away while the 0% HP mob was still alive:
  - **Removed `CurrentHPs <= 0` & `PctHPs <= 0` Death Presumptions**: In EverQuest, low-health mobs (< 1% HP) or mobs with health rounded down often report `PctHPs() == 0` and `CurrentHPs() <= 0` while still standing, casting, and fighting. Liveness is now restored strictly to authoritative death indicators: `not s.Dead()`, `s.Type() ~= 'Corpse'`, and `s.State() ~= 'DEAD'`.
  - **Restored Target Selection & Maintenance (`setTarget`, `isHostileTarget`, `isSpawnAlive`)**: Removed `curHp <= 0` rejection from `runtime.setTarget`, `isHostileTarget`, and `isSpawnAlive`, ensuring the combat engine can acquire, maintain target lock on, and deliver finishing blows to 0% health mobs.
  - **Restored Extended Target & Roam Liveness (`countNPCXtarget`, `findFirstNPCXtarget`, `isXTargetId`, `findRoamTarget`)**: Ensured XTarget discovery, add counters, and puller spawn scanners do not prematurely discard living 0% HP enemies.
  - **Slain Event Specificity (`TriuneSlain1`, `TriuneSlain2`)**: Cleaned up the slain chat event handlers to match the specific slain mob name rather than dropping any active target that happens to be at 0% health.
  - **Updated Regression Suite (`Suite 66`)**: Updated unit tests in `tests/test_pure_logic.lua` to assert that 0% health / 0 HP living mobs remain valid targets and are not flagged as dead.

- **Auto AA Foreign Ability & "1/?" Rank Display Fix (`triune.lua`).** Fixed an issue where abilities belonging to other character classes or unowned ability stubs appeared in the Auto AA progression tab displaying with `1/?` on their rank:
  - **Foreign Ability Stub Rejection (`runtime.isAAAllowedForPlayer`)**: In MacroQuest, unowned abilities or cross-class ability stubs queried via `Me.AltAbility(name)` return a non-zero `ID` and report `Rank = 1` while reporting `MaxRank = 0` (or `nil`). Previously, `isAAAllowedForPlayer` short-circuited if `mid > 0`, erroneously treating all cross-class abilities with an ID as character-owned. Updated the check to strictly verify that `Rank > 0 and MaxRank > 0` before classifying an ability as character-owned, and explicitly reject abilities reporting `Rank > 0 and MaxRank <= 0`.
  - **Strict `maxRank` Enforcement (`runtime.recordScannedAA`)**: Rejected any ability entry where `maxRank <= 0` or missing. Legitimate EverQuest Alternate Advancements always possess a maximum rank >= 1; stubs with missing or non-positive maximum ranks (which rendered as `1/?` in the table) are discarded.
  - **Cache Purging & Discovery Sanitization (`runtime.scanPlayerAAs`)**: Updated Step 1.5 cache pruning to immediately purge any cached abilities that lack a valid `maxRank > 0`. Updated Step 5 `probeRange` to require `MaxRank > 0` and pass `isFromUI = false` so probed abilities are subject to class and archetype restrictions rather than bypassing them.
  - **Table View & Auto-Spend Filtering (`getFilteredSortedAAs`, `checkAutoSpendAA`)**: Added explicit validation to filter out any ability with `maxRank <= 0` from the Auto AA browser table and from automatic training candidate selection.
- **Streamlined Popout Group Window (`triune.lua`).** Cleaned up the visual layout of the popout Group Window (`TriuneGroupWindow`):
  - **Removed Player Class from Member Header**: Streamlined member button headers from `[Lvl <level> <class>] <name>` to `[Lvl <level>] <name>`, reducing visual width and clutter.
  - **Removed Line of Sight (LoS) Indicator**: Removed the `LoS` / `No LoS` text from group member rows, retaining clean distance tracking (`<dist>ft`).
  - **Simplified Health, Mana & Endurance Bars to Percentage Only**: Removed `(cur / max)` raw numbers from the health, mana, and endurance status progress bars, displaying clean percentage totals (`HP: 100%`, `Mana: 100%`, `End: 100%`). Detailed current/max amounts remain accessible in hover tooltips.
- **New Window Settings Tab & Position Persistence System (`triune.lua`).** Added a dedicated **Window Settings** sub-tab under Settings in the main window to save, restore, and automatically recover window positions and sizes across monitor sleep, power cycles, and resolution changes:
  - **Multi-Window Layout Tracking**: Continuously monitors screen coordinates `(X, Y)` and dimensions `(W, H)` for all 9 Triune windows (Main Window, Mini HUD, Target & Player HUD, Group Window, Effects & Songs, Cooldown Monitor, Extended Target, Spell Gem Bar, and Character Stats/Inventory).
  - **1-Click Position Save & Restore**: Save all active window positions directly to the character's persisted loadout (`ctrl.saved_window_positions`) with instant restoral via the UI or `/ac winpos [save|restore|reset]` slash commands (aliases: `/ac savewindows`, `/ac restorewindows`).
  - **Automatic Display Resolution / Monitor Sleep Recovery**: Added `ctrl.winpos_auto_restore_on_resize` which watches viewport display dimensions (`io.DisplaySize`). When turning a monitor off and back on causes Windows or DirectX to collapse and restore resolution, Triune automatically detects the restoration and snaps all windows back to their saved layout.
  - **Window Visibility Synchronization**: Added optional `ctrl.winpos_restore_visibility` toggle to also restore open/closed states when restoring layouts.
  - **Interactive Management Table**: Renders live coordinates, saved coordinates, lock toggles, individual `Center`, `Restore`, and `Save` buttons for each window, plus quick-launch shortcuts for external Triune tools (`triune_inv`, `triune_map`, `triune_spellbook`, `triune_dps`, `triune_cursor`, `triune_buffbot`, `triune_buttons`).
  - **Canonical Version Bump**: Bumped version to **2.10** across `triune.lua` and `README.md`.

- **Comprehensive Item Stat Tooltips (`triune.lua`, `triune_inv.lua`).** Overhauled item hover tooltips across both the popout Character Window and the standalone Inventory Manager to display the full item stat sheet (matching EverQuest's native item inspect display):
  - **Expanded Item Stat Extraction**: `UI.extractCharItemObj` (Character Window) and `extractItemData` (Inventory Manager) now collect all available item TLO attributes: base stats (STR, STA, AGI, DEX, WIS, INT, CHA), heroic stats, spell resists (Magic, Fire, Cold, Disease, Poison, Corruption), HP/Mana/End regen, combat modifiers (Attack, Haste, Accuracy, Avoidance, Combat Effects, Strikethrough, Stun Resist), defensive modifiers (Shielding, Spell Shield, DoT Shield, Damage Shield, DS Mitigation), caster modifiers (Heal Amount, Spell Damage, Clairvoyance), Purity, Instrument Modifier, Required Level, DMG Bonus Type, Tribute value, Worn/Focus/Click spell effects, augment details, and equippable slot names.
  - **Rich Formatted Tooltip Window**: Replaced plain-text `UI.setTooltip()` calls with `ImGui.BeginTooltip()`/`EndTooltip()` rich rendered tooltip windows featuring color-coded sections: gold item names, warning-colored item flags (MAGIC, LORE, NO TRADE, ATTUNEABLE), green stats/pools, arcane-blue resists and caster modifiers, muted location info, and formatted pp/gp/sp/cp currency value display.
  - **Consistent Formatting Across Both Windows**: Identical tooltip layout and color palette in both `triune.lua` (Character Window) and `triune_inv.lua` (Inventory Manager), displaying stats conditionally (only shown when non-zero).

- **Fixed Item Inspect Not Opening Display Window (`triune.lua`).** The `Inspect` context menu option and worn item right-click now reliably open EverQuest's native item display window:
  - **Direct `itemObj.Inspect()` Call**: Replaced the unreliable `/nomodkey /itemnotify <cmd> inspect` command (which silently fails when the item's container bag is closed) with a direct `itemObj.Inspect()` TLO method call that bypasses container state requirements.
  - **Graceful Fallback**: If the direct `Inspect()` call is unavailable (e.g., stale reference), falls back to the `/itemnotify inspect` command.
  - **Item Object Reference Propagation**: The raw MQ item TLO reference (`itemObj`) is now stored in the extracted item table and passed through `ctrl.char_pendingAction` to the main coroutine loop's inspect handler.

---

## 2026-09-07

- **New Popout Character Stats, Inventory & Currency Window (`triune.lua`).** Added a modern single popout window (`TriuneCharacterWindow`) designed to replace EverQuest's default combined Player Stats + Inventory window, featuring three tabs:
  - **Stats Tab**: Character name/level/class/race header with trio class badges, HP / Mana / Endurance progress bars, EXP & AA EXP bars, Base Attributes with Heroic bonuses, all six Resistances, full Combat Stats (Attack, Accuracy, Avoidance, Haste, Shielding, HP/Mana/End Regeneration, Spell Damage, Heal Amount, Damage Shield + Mitigation, DoT Shield, Spell Shield, Strike-through, Combat Effects, Stun Resist), and platinums/gold/silver/copper coin display.
  - **Inventory Tab**: Two-panel layout with the full worn equipment grid (all native slots 0–22) on the left and all personal inventory bags 1–10 on the right, with per-bag used/capacity badges, `Open All` / `Close All` controls, configurable bag column count, and complete item management: left-click to pick up / place / swap on the cursor, right-click to inspect, drag-and-drop between any slots, and a cursor banner with 1-click `Auto-Inventory`.
  - **Alternate Currency Tab**: Live listing of every alternate currency (`Me.AltCurrency`) sorted by value, with an option to hide zero-count currencies.
  - **Non-Blocking Item Actions**: All item actions are queued into `ctrl.char_pendingAction` inside the ImGui draw callback (which never blocks or yields) and executed from the main-run-loop coroutine via `/nomodkey /itemnotify`, with automatic `QuantityWnd` acceptance for stack splits and auto-deferral while casting or in combat.
  - **Right-Click Window Options Menu**: Lock position/size, show zero-count currencies, bag column count (4–12), and opacity slider.
  - **Command & Toolbar Integration**: Added `/ac char` (aliases: `/ac charwin`, `/ac character`, `/ac inventory`, `/ac stats`, `/ac gear`, `/ac currency`) slash command and quick-toggle `Character` / `Char` buttons on the main header toolbar and Mini GUI. Canonical version bumped to 2.09.
...
- **Search/Pull radius read live from the triune loadout; slider removed (`triune_map.lua`, `triune.lua`).** Triune's own control table only contains `camp_radius` (default 100, the "Camp/Pull Radius"), `hunter_radius` (default 1500, the "Search Radius" — the one true hunt/pull search range triune uses at lines 13878/19881/19996/20029), and `hunter_combat_radius` (default 250, the anchor roam radius) — there is no `pull_radius` or `combat_radius` field, so the old map "Search / Pull Radius" slider (seeded from those phantom fields, clamped to 25–600) was driving a circle radius that brave triune never used. The slider is gone; the search/pull/roam circle now renders `td.hunterRadius` straight from `triune_loadout.lua` and the map defaults now match triune exactly (camp 100, search 1500). The map no longer invents its own radius. On triune's side, the three radius sliders (Camp Radius, Search Radius, Combat Radius, Pull Radius) previously assigned `ctrl.*` without persisting — the circumference setting only hit the loadout file on some unrelated later save. Each slider now calls `runtime.saveLoadout(true)` on change (plus `updateMapRadiusVisuals` for the native-map visuals), so moving the slider in triune is immediately visible on the custom map at next sync.

- **Triune camp / pull / anchor / waypoint map overlays restored & expanded (`triune_map.lua`).** The map already synced camp, pull/search, and patrol-waypoint overlays from `triune_loadout.lua` (saved by `triune.lua` into the MQ config dir) — camp anchor pin + camp radius, waypoint nodes/arrival & scan radii with connecting patrol lines, the pull/search roam circle, and hazard hotspots. This change adds the missing Hunter/Puller combat anchor (`hunter_combat_loc` + `hunter_combat_radius`, the "Anchor / Roam Point"): `syncTriuneLoadout` now populates `state.triuneData.hunterAnchor` / `hunterCombatRadius`, the canvas draws a magenta anchor pin with its max-roam radius circle and label, and the pull/search roam circle now anchors at anchor > camp > player (previously camp > player). New persistent "Show Anchor / Roam Point" toggle in Settings & Layers, and the Triune status line reports anchor set/none plus the resolved loadout path. To make the overlays resilient across install layouts, `triune_loadout.lua` discovery now mirrors the multi-path search triune itself uses for `triune_data.lua` (mq.configDir, mq.luaDir, script-relative `TAC/config` / `config`, cwd), and the manual "Sync Triune Data" button prints the exact searched locations and a hint to run `/lua run triune` when the file cannot be found. The triune overlays now draw from a dedicated `drawTriuneOverlays` helper invoked under `pcall` (with a throttled one-per-5s chat error report), so a single malformed overlay datum can never abort the rest of the canvas frame — previously the whole frame after the overlay section (circles *and* NPC markers) silently vanished whenever anything in that block errored, which read as "circles not maintained while the map updates." The pull/search roam circle disappeared every update because `syncTriuneLoadout` re-seeded `ctrl.customSearchRadius` from the loadout on every 2.5s re-sync (clobbering the slider), while the circle itself was anchored at the camp/anchor — so enlarging the radius via the slider would push the circle's edge into view and the next sync would shrink it back off-screen. The circle is now anchored at the player and always visible around the toon, the slider value is respected for the session (`state.searchRadiusTouched`) and never re-clobbered by periodic re-syncs, and it no longer issues per-frame `mq.TLO.Me.X/Y()` fetches (it reuses the cached/interpolated player position passed into `drawTriuneOverlays`). The manual sync also prints a load summary (loadout path, waypoint/camp/anchor/hazard counts). Note: `TAC/config/triune_data.lua` holds spell/AA data only; camp/pull/anchor/waypoints live in `triune_loadout.lua` under the MQ config dir. Verification: `luac -p` clean, main chunk 116 locals (limit 200), all 2,092 unit tests pass, `luacheck` 0 warnings / 0 errors.

- **Smooth player marker & camera-follow update path (`triune_map.lua`).** Player X/Y/Z/Heading are now re-read from TLO every main-loop pass (~40ms / ~25Hz) instead of on a 100ms gate, so player motion is decoupled from the slower NPC scan cadence (500ms). The render callback additionally linearly interpolates the player position between cache writes (`lastPlayer.prevX/Y/Z` + `interpAt`, seeded on first write so the marker never lerps from the world origin), and the interpolated position feeds the player arrow, auto-follow camera center, nav-line origin, and smart-floor bounds — giving fluid map movement/pan even though NPC markers refresh every 500ms. Target ID still rides its 100ms gate.

- **Smooth incremental spawn scanning (`triune_map.lua`).** The periodic `scanZoneSpawns()` rebuilt the full NPC list (up to 120 `NearestSpawn` fetches plus up to 16 `LineOfSight` raycasts) in one synchronous main-loop pass, blocking the ImGui draw callback and making the map visibly chop every scan cycle. The scanner is now an incremental state machine (`state.scanChunk`): the fetch is drained in slices of 12 spawns per frame and the throttled LoS budget in slices of 4 raycasts per frame, with an atomic finalize (filter/sort/group) that swaps `filteredNPCs` only when the cycle completes — so the map keeps rendering the previous valid frame throughout. Init and zone changes pass `forceComplete` for an immediate full pass since they are one-shot events.

- **Fix: Map tool startup lag regression (`triune_map.lua`).** The previously-added color precompute ran `ImGui.GetColorU32` once per map segment/label *at parse time*. A typical map file holds tens of thousands of segment records, so this added tens of thousands of Lua→C++ marshal calls to every zone/map load — delaying window display by 30+ seconds (the draw callback is what actually renders, so the window appeared "missing" until parse finished). Color caching is now computed lazily on a segment's first frame and cached, so parse/load runs zero ImGui calls and renders identically.

- **Map & NPC Tracker Performance Hardening (`triune_map.lua`).**
  - **Navmesh batch cost collapse (`processNavBatch`, hover tooltip)**: The throttled background path verifier no longer computes a full `PathLength` for every NPC in the zone every 6 seconds (two synchronous mq2nav queries per mob, ~100 navmesh calls/sec sustained). It now runs `PathExists` only; the expensive exact path length is resolved on demand for the hovered mob only (throttled to ~1 refresh/sec) and cached so tooltips stay accurate. Cache freshness raised from 6s to 15s (`navState.cacheFreshMs`).
  - **Path verification limited to the visible/needed set (`scanZoneSpawns`)**: Navmesh checks are now enqueued only for mobs that actually pass the current filters (and skipped entirely for remote ATLAS zones), instead of every scanned NPC up to the 120 cap. Mobs still awaiting their first path check are enqueued before the Pathable-Only gate so that filter can't starve itself. `checkQueue` compacts its nil-hole floor.
  - **Spawn scan TLO storm cut (`scanZoneSpawns`, tracker tab)**: Removed the per-mob `LineOfSight()` raycast from every scan — LoS is now resolved lazily through a throttled `losCache` (≤16 raycasts per scan, nearest-first, refreshed on hover) instead of a full raycast per NPC every 300ms. Default scan cadence raised from 300ms to 500ms with a new "Spawn Scan Interval (ms)" slider in Settings & Layers (250–3000ms). Removed six redundant on-keystroke/on-filter `scanZoneSpawns()` calls in the NPC Tracker filter bar (the periodic scan now recomputes filters).
  - **Per-frame TLO churn removed from render callback (`DrawMapCanvas`, `DrawTriuneMapUI`)**: Player X/Y/Z/Heading and target ID are refreshed once per ~100ms from the main loop into `state.lastPlayer`/`state.lastTargetId`; the render callback reads the cache (with a one-time pcall fallback) instead of issuing ~10 cross-language marshals every frame. `Navigation.Active()` reads the scan-populated `navState.navActive`.
  - **Big-zone color precompute (`DrawMapCanvas`)**: Segment and label base colors are computed lazily on first render and cached (`seg.colBase`, `lb.colBase90`), then reused every frame when the Z-filter alpha is 1.0, eliminating a per-segment `ImGui.GetColorU32(float)` conversion every frame on large map files (fallback preserved when alpha is non-opaque). Computed at draw time rather than parse time so map/zone loading performs zero ImGui marshal calls and tool startup stays instant even on atlas meshes with tens of thousands of segments.
  - **Cleanups**: Removed the unused `spawns.allPCs` field; `getConStyle` returns a hoisted fallback constant instead of allocating a new table per miss.
  - **Verification**: `luac -p` clean, main chunk at 103 locals (limit 200), all 2,092 unit tests pass, `luacheck` 0 warnings / 0 errors.

- **Stuck Recovery & Auto-Avoid Stuck Hotspot Hardening (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Hazard Aging & Forgetting (`runtime.decayZoneHazards`, `runtime.recordStuckHazard`)**: Learned navigation hazards no longer accumulate forever. Each hotspot loses one hit every `nav_hazard_decay_minutes` (default 10) it goes without a fresh stuck event, deactivates once below `nav_hazard_min_hits`, and is removed entirely at zero hits. Added a `nav_hazard_max_hits` (default 6) accumulation cap so a bad day can't permanently cripple a zone. Decay runs from the main loop once per minute (`runtime.nextHazardDecayAt`) and persists changes via `saveLoadout`. New "Hazard Forget Time" slider in the Navigation settings UI.
  - **Detour Anti-Oscillation (`runtime.moveToward`, `runtime.moveTowardLoc`)**: Fixed the root cause of detour re-route loops. In-flight detours are no longer hard-killed on a 6s wall-clock expiry while navigation toward the waypoint is still active — they persist on progress and are only abandoned when waypoint navigation actually concludes without arrival (16s backstop). `moveToward` now stamps the waypoint loc key (instead of `nil`) so the delegated `moveTowardLoc` state machine keeps the same detour alive, and a detour with no valid mesh route abandons cleanly to let normal routing proceed. `calculateDetourWaypoint` now penalizes near-side candidates whose onward segment immediately re-crosses a known hazard, preventing circumferential re-route churn.
  - **Dead State Cleanup (`runtime.checkCombatStall`)**: Removed the never-assigned `combatStallSince` and `lastCombatStallRecoveryAt` fields and their six `= nil` writes from `stuckState`/`checkCombatStall`.
  - **Roam Target Hazard Gate Simplification (`findRoamTarget`)**: Collapsed the shadowed `inHaz`/`isMelee`/`pathOk` chain into a single `pathOk = not (inHaz and isMeleeStyle)` guard.
  - **Unit Test Coverage (`Suite 30b`)**: Added deterministic (fake wall-clock) tests for hazard hit capping, timed decay, deactivation below the active threshold, and complete removal at zero hits.

- **Removal of `/alt buy` Dispatch & Cap-Protection MQ2AAspend Delegation Fix (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **All `/alt buy` calls removed (`runtime.startAATrainWorkflow`, `runtime.processAATrainWorkflow`)**: Deleted the fast-path dispatch in `startAATrainWorkflow`, the `select_item` listbox fallback, and the `click_train` belt-and-suspenders call. `/alt buy` does not function on the project's custom server, so it was issuing dead commands and misleading "Issued /alt buy" logs while the real purchase still depended on the window workflow. The notify-based AA Window sequence is now the sole native purchase path; if an ability can't be located in any window list, a warning is logged and the attempt recorded instead of issuing a no-op command.
  - **Cap-threshold delegation no longer skips MQ2AAspend (`runtime.checkAutoSpendAA`)**: The cap-protection branch's fallback condition `(now - lastAACapDelegatedAt) >= 3.0` was always true on first entry (`lastAACapDelegatedAt` unset), so it immediately resorted to the native trainer and never issued `/aaspend`. Added `runtime.lastAACapDelegatedTarget` tracking (mirroring the prioritized-AA `lastAASpendDelegatedTarget` pattern) so the first call delegates to MQ2AAspend and the fallback only fires if points are unchanged after 3 seconds. The new state is cleared in the `finish` step alongside the existing delegation cleanup.
  - **Static analysis cleanup (`tests/test_pure_logic.lua`)**: Replaced three non-narrowing `assert_true(x ~= nil, ...)` guards with Lua's native `assert(x ~= nil, ...)` so the Lua language server no longer reports `need-check-nil` on the subsequent dereferences, and nil failures now abort with the suite label instead of hard-crashing on the following method call.

- **Special Tab Alternate Advancement & Fireworks Auto-Training Fast Path (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Fast-Path `/alt buy` Command Dispatch (`runtime.startAATrainWorkflow`)**: When initiating an AA training workflow for any ability with a known ID (such as *Alternately Advanced Fireworks* with ID 17788), Triune now dispatches `/alt buy <id>` immediately on the fast path. The window workflow continues in parallel for visual client confirmation and secondary fallback.
  - **Unrestricted `/alt buy` Fallbacks (`runtime.processAATrainWorkflow`)**: Removed the obsolete `not runtime.isSpecialTabAA` guards from both the `select_item` listbox fallback and the `click_train` button fallback. Since modern MacroQuest `/alt buy <id>` works reliably across all AA tabs including Special, Special tab abilities now enjoy full `/alt buy` fallback redundancy if listbox indexing is delayed.
  - **Static Analysis Cleanup (`triune.lua`)**: Removed unused `canTrain` and `minLevel` local assignments in `runtime.checkAutoSpendAA`, achieving 0 warnings and 0 errors across all 182 files in `luacheck TAC/`.

- **Between-Pulling Alternate Advancement Purchasing (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Between-Pull Auto-Spend Evaluation (`runtime.pullerTick`, `combatTick`)**: Added hooks in `pullerTick` (Camp mode, `IDLE` state) and `combatTick` (Hunt mode, `not haveNPC`) that check whether any prioritized AA or cap-protection ability is affordable before selecting the next roam target.
  - **Affordability-Guarded Clean Movement Cessation (`runtime.checkAutoSpendAA(allowStop)`)**: Auto-spend strictly checks if an AA can actually be purchased *prior* to altering movement. If an ability is affordable, Triune smoothly halts movement (`stopMoving()`, `/nav stop`, `/stick off`), initiates the window workflow, and pauses pulling. If no AA can be purchased, pulling proceeds immediately without any pause or stutter.
  - **Pulling Movement Hold During Active Training (`runtime.pendingAATrain`)**: Added guards in `pullerTick()` and `combatTick()` (for both Camp, Hunt, and Assist modes) that hold character position while `runtime.pendingAATrain` is active (~0.6s), ensuring client UI interactions complete without movement interruption before immediately resuming pulls.
  - **Motion Grace Period (`runtime.processAATrainWorkflow`)**: Added an `allowStop` grace period (0.35s) to the workflow's motion check, ensuring decelerating characters after pulling do not falsely abort the opening sequence.
  - **Unit Test Coverage (`Suite 73`)**: Added comprehensive unit tests in `tests/test_pure_logic.lua` covering between-pull evaluation, motion cessation, in-combat rejection, unaffordable skip logic, and source code assertions.

- **Alternate Advancement Training Restoration & Anti-Stutter Elimination (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Removed `checkAutoSpendAA` from Active Combat (`combatTick`)**: Eliminated auto-spend execution from the combat tick loop. AA spending now runs exclusively during out-of-combat downtime when stationary, completely stopping in-combat window popups and combat stutter.
  - **Restored Proven AA Window Training Flow (`startAATrainWorkflow`, `processAATrainWorkflow`)**: Restored the clean, direct 5-step AA window sequence (`open` -> `wait_open` -> `prepare_tab` -> `select_item` -> `click_train` -> `finish`). Removed artificial 5-minute skip timers, speculative verification deadlines, and premature `/alt buy` calls that interfered with training Fireworks and custom abilities.
  - **Reliable `wait_open` Step Advancement**: Ensured that the train sequence advances directly to `prepare_tab` upon window open detection or after attempt retries, preventing premature aborts and guaranteed window clicks across custom UI layouts.
  - **Guarded Inventory Fallback**: Strictly verified inventory window state prior to sending fallback button notifications, eliminating red MacroQuest errors (`Window 'InventoryWnd' not available`).
  - **Anti-Pause Movement, Navigation & Casting Protection (`checkAutoSpendAA`, `processAATrainWorkflow`)**: Enforced strict motion gates (`not mq.TLO.Me.Moving()`, `not runtime.isMoveActive()`, `not mq.TLO.Navigation.Active()`, and `not isCasting()`) both in `checkAutoSpendAA` and at the main loop call site. Mid-workflow movement or combat immediately aborts and closes the window cleanly without disrupting character motion.
  - **Endless Retrain Loop Prevention (`runtime.lastAATrainAttempt`)**: Replaced hard lockouts with a lightweight 30-second attempt cooldown that resets automatically whenever unspent AA points change, preventing repeated 2-second window open/close loops on abilities that cannot be trained.
  - **Unit Test Coverage (`Suite 69`, `Suite 72`)**: Validated that all 2,073 unit tests pass with zero failures and main chunk locals remain at 182 (limit 200).

- **Lifetap Spell Healing Logic Isolation & Necromancer Touch of Death Fix (`triune.lua`, `triune_data.lua`, `triune_spellbook.lua`, `tests/test_pure_logic.lua`).**
  - **Early Detrimental Guard in `runtime.isHealAction`**: Added an explicit `if runtime.isDetrimentalAction(name, targetToken, entry) then return false end` guard at the top of `isHealAction`, guaranteeing that offensive lifetap and damage abilities are never mistakenly evaluated by the reactive healing priority engine, gem healing prepends, or healing condition filters.
  - **Condition `my HP <=` Guard Against Hostile Targets (`runtime.conditionMet`)**: Updated the `my HP <=` threshold evaluator so that enemy mob HP is never evaluated when casting detrimental/offensive spells (`E: Current Target`, `isDetrimentalAction`, `isHostileTarget`). When a lifetap is configured with `my HP <=` (e.g. cast defensively when the player drops below 60%), the condition now strictly evaluates the player's own health rather than firing prematurely on low enemy mob HP.
  - **Beneficial Gate in `mapTLOCategoryToKind` (`triune.lua`, `triune_spellbook.lua`)**: Added a strict `bene` requirement to category/subcategory heal string matching (`bene and (catStr:find('heal') ...)`), ensuring detrimental spells with descriptive heal or restore substrings are never classified as friendly heals.
  - **Heuristic Lifetap Coverage in `isDetrimentalSpell`**: Added `lifedraw`, `lifespike`, and `siphon life` to the offline fallback detrimental name patterns alongside `lifetap`.
  - **Database Fix for Necromancer *Touch of Death* (`triune_data.lua`)**: Corrected level 64 Necromancer lifetap *Touch of Death* from `{"Touch of Death",64,1,"heal"}` to `{"Touch of Death",64,0,"dd"}` in the era spell database.
  - **Unit Test Coverage (`tests/test_pure_logic.lua`)**: Added test assertions verifying lifetap rejection from `isHealAction`, `Touch of Death` direct damage classification in `triune_data.lua`, and `my HP <=` player-only evaluation on enemy targets.


- **Custom Server AA Compatibility & Removal of Speculative Static Pre-Checks (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Eliminated Speculative Pre-Checks (`runtime.checkAutoSpendAA`)**: Removed the static `ga.CanTrain()` and `MinLevel` pre-check that previously caused custom/special server abilities (such as *Bestial Frenzy*) to be falsely marked with `Unmet Req` and placed on a 5-minute skip. On custom servers that alter level requirements, prerequisite trees, or class access, eligibility is now determined strictly through live client interactions and server purchase verification rather than static client database tables.
  - **Unconditional User Priority Authorization (`runtime.isAAAllowedForPlayer`)**: Added explicit bypasses in `isAAAllowedForPlayer` so that any ability checked by the user in `ctrl.auto_aa_priorities` or discovered with an in-game ID is unconditionally permitted, guaranteeing that custom server abilities are never rejected or pruned from the loadout.

- **Alternate Advancement In-Game Description Tooltips & Text Wrapping (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **AA Description Extraction & Caching (`runtime.getAADescription`, `runtime.recordScannedAA`)**: Integrated in-game ability descriptions from MacroQuest's `mq.TLO.Me.AltAbility(name).Description()` and `mq.TLO.AltAbility(name).Description()` (with spell fallback for activated AAs). Descriptions are cached both in memory on the ability entry and persisted in `runtime.cachedAAData` across sessions.
  - **Clean Word-Boundary Text Wrapping (`runtime.wrapText`)**: Implemented a formatting helper that formats long descriptions across multiple lines at word boundaries (~55 characters) while preserving existing paragraphs, preventing tooltips from spanning excessively wide across the screen.
  - **Hover Tooltips in Auto AA Table (`runtime.showAATooltip`, `UI.drawAutoAATab`)**: Hovering over an ability name or rank in the Auto AA browser displays a comprehensive tooltip showing the ability name, current rank, maximum rank, next rank cost, points spent, followed by the full wrapped description. Implemented using percent-safe string formatting (`ImGui.SetTooltip('%s', ...)`).
  - **Automated Regression Suite (`Suite 71`)**: Added unit tests in `tests/test_pure_logic.lua` covering text wrapping, lazy description extraction and caching, percent format string safety, and UI source code inspection.

- **Player-Specific AA Filtering & Cross-Class Ability Rejection (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Class & Archetype Ability Maps (`runtime.CLASS_ARCHETYPES`, `runtime.ARCHETYPE_CLASSES`, `runtime.ARCHETYPE_RESTRICTIONS`, `runtime.CLASS_SPECIFIC_ABILITIES`, `runtime.buildAAClassRestrictions`)**: Mapped all 16 EverQuest classes and archetypes to their class-specific and archetype-specific Alternate Advancement abilities.
  - **Player Ability Validator (`runtime.isAAAllowedForPlayer`)**: Implemented strict validation ensuring that characters only see and auto-train AAs belonging to their own classes (`myClasses`), their archetype (Melee, Priest, Caster, Pet), universal General AAs, or Special tab abilities. Rejects cross-class abilities (e.g. Warriors seeing `Headshot`, `Mana Burn`, `Harm Touch`, `Divine Arbitration`, or Clerics seeing `Taunt`, `Area Taunt`, `Ambidexterity`).
  - **Permissive Ownership & In-Game UI Bypasses**: Grants immediate access if the character has already trained rank > 0 or if the ability was scanned directly from the character's live in-game `AAWindow` listbox.
  - **Cross-Class AA Cache Pruning (`runtime.scanPlayerAAs`)**: Updated Step 1.5 in the AA scanner to automatically prune any stale or cross-class abilities from `runtime.cachedAAData`, immediately cleaning up characters whose caches were populated by earlier scans.
  - **Automated Regression Suite (`Suite 70`)**: Added comprehensive unit tests in `tests/test_pure_logic.lua` validating player-specific AA filtering, cross-class rejection for Warrior and Cleric profiles, ownership bypasses, and cache pruning.

- **Alternate Advancement Purchase Verification & Unpurchasable Ability Anti-Stall (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Purchase Outcome Verification (`runtime.processAATrainWorkflow`)**: Added initial point and rank tracking (`task.initialPoints`, `task.initialRank`) when initiating an AA training workflow. After clicking the Train button (or issuing fallback `/alt buy`), the workflow transitions to a new `verify` step that checks whether unspent AA points decreased (`currentPts < initialPts`) or ability rank increased (`currentRank > initialRank`).
  - **Unpurchasable Ability Skip & Cooldown (`runtime.aaSkipUntil`)**: If an AA purchase cannot be completed (e.g. `AAW_TrainButton` disabled in UI, level requirement not met, missing prerequisites, or server rejected purchase), Triune records the failure and temporarily skips that ability for 5 minutes (`runtime.aaSkipUntil[name] = now + 300`). This ensures Auto AA never stalls or loops indefinitely on an unpurchasable ability and seamlessly advances to the next prioritized candidate.
  - **UI Train Button Disabled State Detection**: In `click_train`, Triune now checks `btn.Enabled()` on the client's `AAW_TrainButton`. If EverQuest has disabled the train button, Triune avoids unnecessary clicks, notes the unmet requirement, and flags the ability for skip cooldown.
  - **Level-Up & Manual Spend Skip Reset (`runtime.checkAutoSpendAA`, `runtime.manualSpendAA`)**: Whenever the character levels up (`mq.TLO.Me.Level()` change), all temporary AA skips are automatically cleared so newly unlocked abilities can be purchased immediately. In addition, manual `Train##btn` or `Spend Now` button clicks immediately clear skips for the selected ability.
  - **UI Unmet Requirements Status Indicator (`UI.drawAutoAATab`)**: In the Auto AA table, abilities currently under temporary skip cooldown display a yellow `Unmet Req` status with a descriptive hover tooltip explaining that the ability is being skipped due to level/prerequisite requirements.
  - **Automated Regression Suite (`Suite 69`)**: Added comprehensive unit tests in `tests/test_pure_logic.lua` validating purchase verification, skip timeout placement, advancement to subsequent priority candidates, level-up reset handling, manual spend overrides, and static pre-checks.

- **Alternate Advancement Auto-Spend MQ2AAspend Fallback & UI Delegation Toggle (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Automated Native Window Training Fallback (`runtime.checkAutoSpendAA`)**: Resolved an issue where auto-spending stalled when delegating to `MQ2AAspend` on progression/emulated servers where `/alt buy` or `eqlib::AltAdvManager::CanTrainAbility` checks fail. If Triune delegates an AA purchase to `MQ2AAspend` and the ability remains unpurchased with unspent points unchanged after >= 2.5s, Triune automatically falls back to its built-in native window trainer (`runtime.startAATrainWorkflow`) to train the prioritized ability using in-game window UI clicks.
  - **Cap Dump Protection Fallback (`runtime.checkAutoSpendAA`)**: Extended the fallback mechanism to cap dump protection (e.g. Fireworks or designated cap dumps) if `MQ2AAspend` fails to spend points when at or above the threshold.
  - **Direct Spend Now Prioritized Fallback (`runtime.manualSpendAA`)**: Updated `Spend Now` button handling so that clicking `Spend Now` with prioritized abilities immediately purchases the top affordable prioritized ability (or falls back to native training if `MQ2AAspend` previously stalled).
  - **Interactive UI Delegation Toggle (`UI.drawAutoAATab`)**: Converted the static `MQ2AAspend` indicator into an interactive checkbox (`MQ2AAspend##aaDelegateMaster`) in the AA Progression header toolbar. Users can now directly toggle between delegating to `MQ2AAspend` or using Triune's native window trainer without needing slash commands.
  - **Automated Regression Suite (`Suite 64`)**: Enhanced unit tests in `tests/test_pure_logic.lua` covering stalled `MQ2AAspend` delegation detection, native fallback triggers, direct native spending with delegation disabled, and UI toggle token verification.

- **Native Alternate Advancement Window Trainer & Window Control Overhaul (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Window Object Call & Boolean Bugfix (`runtime.getAAWindow`, `runtime.openAAWindow`, `runtime.closeAAWindow`)**: Resolved a critical issue where Triune logged `Initiating AA Window train sequence...` but never actually opened the in-game Alternate Advancement window. In MacroQuest Lua, calling a Window TLO as a function (`w()`) invokes `ToString()`, which returns boolean `false` when the window is closed. Previously, statements like `if w and w() and w.DoOpen then w.DoOpen() end` evaluated to `false` whenever the window was closed, preventing `DoOpen()` and `DoClose()` from ever firing. Created clean dedicated window helpers (`runtime.getAAWindow`, `runtime.getAAWindowName`, `runtime.isAAWindowOpen`, `runtime.openAAWindow`, and `runtime.closeAAWindow`) that query `.Open()` and invoke `.DoOpen()` without testing `w()`.
  - **Native EQ Command Keypress (`TOGGLE_ALTADVWIN`)**: Replaced non-functional `/keypress V` with EverQuest's native internal command `/keypress TOGGLE_ALTADVWIN` and standard key combos (`/keypress v alt`, `/keypress a alt`), as well as `/notify InventoryWindow IW_AltAdvBtn leftmouseup` (and `InventoryWnd`), ensuring the AA window opens reliably across client configurations, UI skins, and keybind layouts.
  - **Window Name Resolution (`AAWindow` & `AAWnd`)**: Supported both `AAWindow` and custom UI alias `AAWnd` across all notifications, list queries, and window operations.
  - **Recursive Child Window Iteration Bugfix (`runtime.findChildRecursive`)**: Resolved an issue where `findChildRecursive` prematurely terminated child traversal when encountering an inactive or closed tab page (`while curr and curr()`). Changed loop check to `while curr and safety < 120`, allowing seamless navigation across inactive tab pages and deeper UI hierarchies to find listboxes and buttons.
  - **Safe Workflow Abort on Failed Open (`runtime.processAATrainWorkflow`)**: Replaced premature step advancement with a clean retry threshold (up to 4 attempts) that outputs a clear warning and terminates safely if the client window fails to open, rather than executing phantom list clicks or issuing rejected `/alt buy 0` commands.
  - **Eliminated Toggle Collisions & Red `InventoryWnd` Notifications (`runtime.openAAWindow`)**: Fixed an issue where all opening methods were fired simultaneously in a single function call. While non-toggling `/windowstate AAWindow open` successfully opened the window, immediate subsequent keypresses (`TOGGLE_ALTADVWIN`, `v alt`, `a alt`) acted as toggles that immediately shut the window back down, while blind `/notify InventoryWnd IW_AltAdvBtn` commands triggered red MacroQuest errors (`Window 'InventoryWnd' not available`). Refactored `openAAWindow(attempt)` to staged attempts: Attempt 1 executes non-toggling `/windowstate %s open` and `DoOpen()`, subsequent retry attempts issue fallback toggle keypresses only if needed, and inventory button notifications are strictly guarded to only fire when the player's inventory window is actually open.
  - **Direct Open State Detection (`runtime.isAAWindowOpen`)**: Updated `isAAWindowOpen()` to check `mq.TLO.Window('AAWindow').Open()` and `'AAWnd'` directly, eliminating false-negative window state evaluations.
- **Comprehensive Auto AA Discovery & Unpurchased Ability Retention (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Removed `isAAWindowOpen()` Scan Gate**: Discovered that `scanPlayerAAs()` previously wrapped listbox reading (`AAW_GeneralList`, `AAW_ArchList`, `AAW_ClassList`, `AAW_SpecialList`) inside `if runtime.isAAWindowOpen() then`. Because players normally play with the in-game AA window closed, Step 1 was skipped 100% of the time during routine gameplay. Removed this gate and the `child()` boolean check, reading listbox children loaded in EQ memory directly.
  - **Removed Non-UI `id <= 0` Rejection in `recordScannedAA`**: On EverQuest emulated/progression servers, `mq.TLO.Me.AltAbility(name)` returns `nil` for unpurchased (Rank 0) abilities, and unowned passive abilities return ID 0. Previously, `if not isFromUI and id <= 0 then return end` dropped all unpurchased AAs scanned from catalogs or config when the window was closed. Removed this check while continuing to strictly reject character skills (`Mend`, `Flying Kick`, `Backstab`) via `mq.TLO.Skill(name)`.
  - **Reinstated Norrath AA Catalog (`NORRATH_AAS`)**: Added a curated reference catalog of General, Archetype, and Class AAs (including `Combat Agility`, `Combat Stability`, `Physical Enhancement`, `Fury of Magic`, `Twinproc`, `Extended Ingenuity`, `Fearless`, `Bloodlust`, `Gelid Rending`, `Run Speed`, etc.) to ensure that all core progression abilities are immediately listed and prioritizeable even prior to first purchase or before the client UI listboxes populate.
  - **Step 1.5 Cache Pruning Bugfix**: Updated cached AA verification so that abilities with `id == 0` and `maxRank == 0` are not purged from `runtime.cachedAAData` unless they are detected as true character skills.
- **Out-of-Combat AA Spending Enforcement & Deep UI Hierarchy Traversal Fix (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Strict Out-of-Combat Spending Gate (`runtime.checkAutoSpendAA`)**: Enforced a strict out-of-combat gate across all automatic Alternate Advancement spending (`mq.TLO.Me.Combat()`, `mq.TLO.Me.CombatState() == 'COMBAT'`, `AutoFire()`, `anyXtarAlive()`, `XTHaterCount() > 0`). Triune will now never trigger AA window workflows or point purchases during combat encounters, completely eliminating pauses, stalls, and player interruptions while fighting.
  - **Mid-Train Combat Abort Handler (`runtime.processAATrainWorkflow`)**: Added an immediate aggro and combat check inside `processAATrainWorkflow`. If combat initiates while an AA train sequence is active, Triune immediately closes the AA window and aborts the workflow cleanly to prioritize combat.
  - **Deep UI Hierarchy Traversal Bugfix (`runtime.findChildRecursive`)**: Resolved an issue where `findChildRecursive` failed to search nested tab pages and subwindows. Previously, recursion was gated behind `curr.Children and (curr.Children() == true)`. Because MacroQuest TLO booleans return DataType objects or string `"TRUE"`, `"TRUE" == true` evaluated to `false` in Lua, preventing the traversal from descending down into child tab boxes, subwindows, and pages (`AAWindow -> AAW_Subwindows -> AAW_GeneralPage -> AAW_GeneralList`). Refactored to inspect `curr.FirstChild` directly, enabling full recursive discovery of all listboxes and buttons.
  - **Enhanced AA Row Selection & Train Button Execution (`runtime.findAAInWindowLists`, `runtime.processAATrainWorkflow`)**: Added direct text matching via `child.List('=' .. name)` before falling back to row iteration, with safe extraction across string, userdata, and function types. Pre-selected initial tab pages using known AA categories (`General`, `Archetype`, `Class`, `Special`), added a 0.2s tab activation delay upon switching tabs, and updated button clicks to fire both `LeftMouseDown` and `LeftMouseUp` events and notifications so the client engine registers the purchase.
  - **Automated Regression Suite (`Suite 68`)**: Added unit tests covering in-combat AA spending prevention, 3-level deep nested UI tree navigation, and code token verification.
...
- **Out-of-Combat Self-Healing & Beneficial Target Selection Fix (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Differentiated In-Combat vs. Out-of-Combat Beneficial Targeting (`castGem`, `fireAA`, `useClickie`)**: In EverQuest, casting beneficial spells on yourself while actively fighting an enemy mob (`hostileTarget`) automatically directs the spell to yourself without dropping your combat target. However, out of combat (or when no hostile mob is selected, such as idle with no target or targeting a neutral NPC/player), EverQuest strictly requires you to explicitly select yourself (`orig == mq.TLO.Me.ID()`) to cast the spell.
  - **Fixed `needsTarget` Condition**: Resolved an issue where `needsTarget = (not selfCast) or (not hostileTarget and orig > 0 and orig ~= id)` failed to target the player when out of combat with no active target (`orig == 0`), causing self-heals, self-buffs, and clickies to fail with "You must target someone to cast this spell!". Unified the targeting requirement across spells, AAs, and clickies to `needsTarget = (orig ~= id) and not (selfCast and hostileTarget)`.
  - **Cast Target Retention (`castTracker.targetRequired`)**: Updated `castTracker.targetRequired` in `castGem`, `fireAA`, and `useClickie` to properly flag single-target beneficial spells as target-required when casting out of combat (`not (selfCast and hostileTarget)`), allowing `getActiveTargetRequiredCastingId()` to protect target lock on self during the cast duration.
  - **Target Restoration (`restoreTargetId`)**: Preserved previous non-hostile targets (`orig > 0` and `not (selfCast and hostileTarget)`) via `restoreTargetId` so that casting out-of-combat self-heals or buffs seamlessly restores the player's previous target (e.g. an NPC vendor, quest giver, or group member) upon cast completion.
  - **Automated Regression Suite (`Section 46`)**: Added unit test assertions in `tests/test_pure_logic.lua` covering out-of-combat self-heals with no target, self-heals with non-hostile targets, and in-combat hostile target retention.

- **Unreachable Target Abandonment & Pursuit Stall Watchdog Fix (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Comprehensive Pursuit Stall Give-Up (`moveToward`)**: Expanded pursuit stall give-up (`PURSUIT_STALL_TIMEOUT` of 8s or 3 navigation stalls) beyond `Puller` mode to all combat modes (when `not followOnly`). When stalling, Triune now immediately executes `clearTarget()`, sets `runtime.pullState = 'IDLE'` and `runtime.pullTargetId = 0`, and clears navigation caches, preventing the target ring from holding the unreachable mob and endlessly resetting the stall timer on the very next tick.
  - **Direct Navmesh Path Pre-Validation & Failure Handling (`moveToward`)**: In Stage 1 MQ2Nav navigation, when the zone navmesh is confirmed loaded and `mq.TLO.Navigation.PathExists('id ' .. id)()` returns false for a distant/non-LoS target, Triune no longer falls through to blind `/keypress forward hold` or `/stick` straight into walls. If a path fails for 3 consecutive checks, it marks the target unreachable, stops movement, and clears target immediately.
  - **Fixed 15-Second Pursuit / Approach Watchdog (`combatTick`)**: Resolved a critical bug where the 15-second unreachable timeout in `combatTick` required `engage or Me.Combat() or Me.AutoFire()`, which was always false while running toward an unreached mob and reset the timer every tick. Replaced it with an active approach watchdog (`pursuit.approachStartedAt`) that resets when within striking reach or actively engaged with LoS, and times out after 15s of failing to close the gap.
  - **Repositioning & Cannot-Hit Event State Preservation (`repositionCloser`, `handleCantHitFromHere`)**: Removed destructive `pursuit.id = 0` resets from repositioning event handlers that previously wiped stall detection whenever chat reported "too far away" or "cannot hit from here". Added 4-attempt abandonment logic to `handleCantHitFromHere` to abandon obstructed targets after repeated failed reposition maneuvers.
  - **Close-Range Roam Path Checking (`findRoamTarget`)**: Removed the hardcoded `dist > 25` bypass in `findRoamTarget`, enforcing navmesh path validation whenever a target is outside immediate close reach or lacks line of sight.
  - **Automated Regression Suite (`Suite 65`)**: Added 17 unit test assertions validating unreachable TTL, XTarget filtering, approach watchdog timing, and source logic token integrity.

- **Special Tab Alternate Advancement Native Training & MQ2AAspend Routing Fix (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Special Tab Detection (`runtime.isSpecialTabAA`)**: Added explicit detection for Special tab abilities (including *Alternately Advanced Fireworks*, glyphs, and abilities cataloged under category `'Special'`).
  - **Bypassed MQ2AAspend for Special Tab Abilities (`runtime.checkAutoSpendAA`, `runtime.manualSpendAA`)**: Resolved an issue where delegating auto-spend to `MQ2AAspend` caused Special tab abilities to never purchase. `MQ2AAspend` only supports General, Archetype, and Class tabs and reports Special tab abilities as unavailable. When a prioritized or cap-dumping ability is located on the Special tab, Triune now automatically routes it directly to its native window training workflow (`runtime.startAATrainWorkflow`), leaving standard progression AAs to `MQ2AAspend`.
  - **Direct Manual `[Train]` Button Execution (`UI.drawAutoAATab`, `runtime.manualSpendAA`)**: Clicking the `[Train]` button on a specific ability row in the AA browser table now directly and unconditionally invokes `runtime.startAATrainWorkflow(targetName)` to purchase that specific ability natively, rather than executing a generic `/aaspend auto now` command that fails on Special tab items.
  - **INI Exclusions for Unsupported Abilities (`runtime.syncAAsToMQ2AASpendIni`)**: Excluded Special tab abilities from being written to `[MQ2AASpend_AAList]` in the character's `.ini` file, preventing `MQ2AAspend` from getting blocked or reporting invalid format errors on abilities it cannot purchase.
  - **Robust UI Tab & Control Selection (`runtime.processAATrainWorkflow`)**: Enhanced the `prepare_tab` step to issue tab selection notifications to both `AAW_Subwindows` and `Subwindows` controls (`/notify AAWindow AAW_Subwindows tabselect 4` and `/notify AAWindow Subwindows tabselect 4`), ensuring reliable activation of Tab 4 (Special tab) across custom UI skins.
  - **Automated Regression Suite (`Suite 64`)**: Expanded test assertions to 27, validating Special tab identification, INI filtering, and native purchase routing.

- **Auto AA Ability Discovery & Accurate Cost Resolution Overhaul (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Comprehensive Untrained Ability Discovery (`runtime.recordScannedAA`, `runtime.scanPlayerAAs`)**: Fixed an issue where unpurchased/untrained character Alternate Advancements (rank 0) were discarded from the Auto AA tab because `mq.TLO.Me.AltAbility(name)` returns nil for unowned abilities. Added multi-tiered discovery scanning across character classes in `DATA.aas`, standard Archetype/General ability catalogs (`COMMON_AAS`), class-specific catalogs (`CLASS_AAS`), and `AAWindow` UI listboxes.
  - **Recursive `AAWindow` Inspection (`runtime.findChildRecursive`, `runtime.scanPlayerAAs`)**: Replaced shallow window control searches with recursive descendant searching across all standard client listboxes (`AAW_GeneralList`, `AAW_ArchList`, `AAW_ClassList`, and `AAW_SpecialList`). Fixed incorrect window control identifier (`AAW_ArchList` was previously queried as `AAW_ArchetypeList`).
  - **Accurate Cost & Rank Extraction**: Extracted columns 2 (`Cur/Max`, e.g. `0/3`) and 3 (`Cost`, e.g. `3`) directly from client listboxes, queried global `mq.TLO.AltAbility(name).Cost()` and `MaxRank()`, and guaranteed that untrained abilities display accurate costs and ranks (`0/3`) rather than `-` or 0 costs.
  - **Persistent AA Cache (`runtime.cachedAAData`)**: Implemented persistent memory across scans so that abilities discovered via UI or database retain their cost, max rank, and category even when closed or not currently selected in the client.
  - **Automated & Manual Purchasing Fixes (`runtime.checkAutoSpendAA`, `runtime.manualSpendAA`)**: Enhanced auto-spending and manual train buttons to resolve actual costs for unpurchased abilities from cache and global TLO, allowing players to purchase rank 1 of unowned abilities rather than failing on nil checks or falling back to a 25 AA threshold.
  - **Client Keypress Safety (`runtime.processAATrainWorkflow`)**: Replaced invalid `/keypress alt_advancement` with standard EverQuest `/keypress V` when opening the Alternate Advancement window during automated training.
  - **Automated Regression Suite (`Suite 63`)**: Added 14 unit test assertions validating untrained ability preservation, cache persistence across scans, rank/cost updating, maxed status resolution, and non-zero fallback cost guarantees.
...
- **In-Game LLM Test Runner & Autonomous QA Agent (`triune_test.lua`, `triune_llm_bridge.py`, `start_bridge.sh`, `start_bridge.bat`, `tests/check_theme_consistency.sh`, `README.md`).**
  ...
  - **Authentic `/ac` Command Cheatsheet & Strict Model Grounding**: Embedded the complete, authentic Triune `/ac` command reference directly into the agent's system prompt (Execution, Modes & Submodes, Combat Styles & Ranges, Main Assist, Spells & Lockouts, Trio Pets, Hunter/Puller configuration, Waypoints, AA Auto-Spending, and Utilities). Explicitly prohibits models from hallucinating non-existent modes (such as `/ac mode tank` or `/ac mode kite`).
  - **Expanded 14-Suite Pre-Built Test Matrix & Combo Dropdown UI**: Expanded pre-built test suites from basic checks to 14 dedicated test suites covering all operational areas of TriuneAutocombat (Modes, Assist Trio Roles, Engine Execution/Burn, Styles/Range, Gem Import & Auto-Mem, Auto AA Spending, Pet Management, Main Assist & Defense, Puller Con Filters, Hunter Z-Planes, Waypoint Patrol, Loadout Presets, Target Telemetry, and Full Control Surface Regression). Replaced sprawling radio buttons with a sleek, compact `ImGui.BeginCombo` dropdown.
  - **Multi-Turn Conversation History Preservation**: Implemented `State.conversationHistory` tracking across testing steps (`QUERY`, `DELAY`, `ASSERT`) so subsequent prompts preserve the agent's thoughts, action history, and system instructions instead of losing context mid-run.
  - **Launcher Scripts & Theme Verification**: Added `start_bridge.sh` and `start_bridge.bat` for one-click daemon startup, integrated `triune_test.lua` into `tests/check_theme_consistency.sh` to ensure strict theme parity, and resolved all `luacheck` warnings (0 warnings / 0 errors across 182 files).
- **Complete Removal of Retired Updater Scripts & References (`.github/workflows/ci.yml`, `.github/workflows/release.yml`, `triune.lua`, `README.md`, `.agents/AGENTS.md`, `tests/test_pure_logic.lua`).**
  - **CI & Release Workflows (`ci.yml`, `release.yml`)**: Removed the retired `triune_updater.lua` grep step from the `version-sync` CI job, which was causing CI runs to fail on missing file. Updated release archive packaging in `release.yml` to remove non-existent `triune_updater.py`, `update.bat`, and `update.sh` paths.
  - **Engine Slash Command Cleanup (`triune.lua`)**: Fully excised `/ac update` and its aliases (`/ac updater`, `/ac checkupdate`) from the command dispatcher, Help tab command reference, help chat printout, and `/ac` error usage summary string.
  - **Documentation & Agent Rules Alignment (`README.md`, `.agents/AGENTS.md`)**: Removed the Release Updater row and `/ac update` command from the documentation tables and excised deleted updater files from the directory tree. Aligned `.agents/AGENTS.md` File Map and version consistency guidelines.
  - **Test Suite Cleanup (`tests/test_pure_logic.lua`)**: Completely deleted Suite 41 (`triune_updater release parsing & JSON tokenizer`), keeping unit test suites 100% focused on active codebase features.
