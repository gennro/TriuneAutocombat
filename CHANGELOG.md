...
- **MQ2AAspend Plugin Autoloading, INI Auto-Sync & Streamlined Auto AA UI (`triune.lua`, `tests/test_pure_logic.lua`).**
  - **Automatic Plugin Autoloading (`runtime.autoloadRequiredPlugins`, `runtime.checkAutoSpendAA`)**: `MQ2AAspend` is now automatically loaded on startup alongside `mq2nav` and `mq2moveutils` (`/plugin mq2aaspend load`), and will automatically attempt to reload if unloaded during auto-spend evaluation.
  - **Zero-Friction Default Delegation**: Enabled `ctrl.auto_aa_delegate_aaspend = true` by default. When the player checks `[x] Auto-Spend AA`, Triune immediately ensures the plugin is loaded, syncs priorities to the character's INI, and handles background purchases.
  - **Streamlined Unified Toolbar (`UI.drawAutoAATab`)**: Consolidated the Auto AA tab into a single, clean header: live pool status, master `[x] Auto-Spend AA` checkbox, `● MQ2AAspend` live status badge, quick `Bank: [ 25 ]` threshold slider, `[Spend Now]`, `[Sync to INI]`, and `[↻ Refresh]`. Removed unnecessary duplicate settings and manual configuration hurdles.
  - **Automatic Real-Time INI Synchronization (`runtime.syncAAsToMQ2AASpendIni`)**: Every checkbox click in the AA browser table automatically rewrites `[MQ2AASpend_AAList]` in `<Server>_<Character>.ini` (`1=Ability Name|M`, etc.) and reloads the plugin via `/aaspend load` with zero manual intervention.
  - **Purged Combat Skills from AA List**: Completely removed hardcoded catalogs (`COMMON_AAS` and `CLASS_AAS`) that accidentally introduced normal character combat skills into the AA list. Added strict `mq.TLO.Skill(name)` rejection and valid AltAbility ID checks.
  - **Automated Regression Suite (`Suite 64`)**: 19 unit test assertions verifying skill rejection, non-UI invalid ability exclusion, threshold-gated command dispatching, `MQ2AASpend_AAList` INI formatting (`|M`), and source code token verification.
