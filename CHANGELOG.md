...
- **Fix Ranged Auto-Attack Getting Stuck On Target Switch (`triune.lua`).**
  - **Root Cause**: Under this server's `#attackmode ranged` scheme, `/attack` behaves as a pure on/off toggle rather than "start attacking my current target." If the toggle is already "on" from a just-killed mob, sending `/attack on` again for the newly-engaged target is a no-op -- the character stops firing entirely until the toggle is manually cycled off and back on. Every Ranged engagement site only ever checked `not Me.Combat()` before sending `/attack on`, which is blind to this: `Me.Combat()` kept reporting true across the target switch even though nothing was actually firing.
  - **`ensureRangedAutoAttack(tid)`**: New shared helper (`runtime.lastRangedAttackTargetId`) that detects an actual target change and forces a real `/attack off` -> (short beat) -> `/attack on` cycle instead of trusting the current toggle state. Same-target ticks fall back to the original "turn it on if it's off" check. Wired into all four places that drive Ranged auto-attack: `checkCombatStall()`, `combatTick()`'s main Ranged engage block, and both puller code paths (`pullerTick()`'s Camp-mode pull and `combatTick()`'s Hunter-mode pull).
  - **Follow-up fix (same day)**: the first version of `ensureRangedAutoAttack` spread the off/on cycle across two ticks with a pending flag (`runtime.rangedRetoggleAt`), requiring the function to be called again ~0.3s later to send the completing `/attack on`. Since every call site is gated behind distance/casting/target checks that can go false on the very next tick, the completing call sometimes never happened, leaving the character with combat toggled off entirely on some target switches -- worse than the original bug. Rewrote it to block synchronously with `mq.delay(300)` between the off and on commands inside a single call, so the cycle always completes before the function returns. Safe here because `ensureRangedAutoAttack` only ever runs from the main-loop coroutine (`combatTick`/`checkCombatStall`/`pullerTick`), never from the ImGui render callback. `runtime.rangedRetoggleAt` removed as dead state.
  - **Second follow-up fix (same day)**: `combatTick()`'s Hunter-mode roam-pull `pullStyle == 'Ranged'` branch had a shortcut -- `if isXTargetId(id) or Me.Combat() then engage = true end` -- that returned without ever calling `ensureRangedAutoAttack`. When mobs are clustered close together, the next pull target is often already flagged as "in combat" (carried over from the kill that just happened, or from being hit in melee the instant it's tagged), so this shortcut fired on effectively every tick and the target-change retoggle for the new mob never ran -- the character would just stand there not firing until `/attack` was cycled manually. Now routes through `ensureRangedAutoAttack(id)` inside that shortcut too (when `combat_style == 'Ranged'` and `serverAttackMode == 'Ranged'`), so a genuine target change is still caught even when the code already believes it's engaged.

- **Lower Minimum Ranged/Spell Combat Distance From 15 To 5 (`triune.lua`, `README.md`).**
  - The Settings tab's "Combat Distance" slider (`ctrl.ranged_dist`, shown for Ranged/Spell combat styles) had a hardcoded minimum of 15, forcing hybrid builds that actually want to fight at close/melee range while in Ranged or Spell style to constantly back out to 15 units instead of holding position. Lowered the slider floor, the `/ac range` command's clamp, and `repositionCloser()`'s "target too far away" reposition floor all from 15 to 5 (matching the existing Melee Distance floor). `README.md` command table updated to reflect the new 5-200 range.

## 2026-08-23

- **Ranged Pulling: Server-Native Attack Mode + Aggressive Close-and-Fire (`triune.lua`).**
  - **`#attackmode ranged` Instead of `/autofire` During Pulls**: Both puller code paths (`pullerTick()`'s Camp-mode `TO_MOB` state, and `combatTick()`'s Hunter-mode roam pull) now check `ctrl.combat_style` before choosing how to fire a bow while pulling. When AutoCombat is set to Ranged, `/autofire` leaves the character not attacking at all on this server, so pulling now uses the same server `#attackmode ranged` + plain `/attack on` approach as general Ranged combat engagement, tracked via `runtime.serverAttackMode`. When `pull_style == 'Ranged'` is combined with a Melee/Spell combat style (bow-tag then melee/spell), `/autofire` is unaffected and still used exactly as before.
  - **Close to Engagement Range While Already Firing**: Ranged pulling previously approached only to the looser `ctrl.pull_engage_dist` (default 100) and stalled there, waiting stationary for the mob to close the gap before ever reaching effective bow range. `reqRange` for `pull_style == 'Ranged'` now targets `ctrl.ranged_dist` directly (or the Stand Back distance, when `ctrl.pull_stand_back` is enabled), so the character keeps closing distance toward true engagement range while already attacking, rather than pausing short and standing still.
  - **`pullerTick()` Attack-Trigger Decoupled from Full Arrival**: The Camp-mode `TO_MOB` state previously gated all pull-tagging logic (facing, firing, pet/spell dispatch) behind full arrival at `reqRange`, so nothing happened at all until movement fully stopped. Restructured to match the Hunter-mode pattern: movement and the attack-trigger check now run independently each tick, so Ranged pulls start firing as soon as within the outer `pull_engage_dist` window (same threshold as before) while movement keeps closing in underneath it.
